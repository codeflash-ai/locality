...
Goal: generate the same launch-readiness report using local git commands plus Notion MCP for Notion context.

Use these context sources:
- local git commands in `REPO_DIR`
- Notion MCP tools for Notion search/read context

Do not read mounted Locality Notion files under `{{SANDBOX_HOME}}/notion`.
Do not use `loc` commands.
Do not create Notion pages/docs, push or update Notion, or update any remote source.

Required work:
1. Inspect relevant commits with local git commands as needed.
2. Use Notion MCP to search/read context around:
...
3. Read the benchmark case section in the Locality Launch Amika Environment page through Notion MCP.
4. Write the final Markdown report to `OUT_DIR/notion-mcp-report-body.md`.
