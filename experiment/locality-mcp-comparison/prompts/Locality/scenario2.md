...
Required work:
1. Inspect relevant recent code changes with git, and use `gh` only for repository or issue context that materially affects the memo.
2. Read the hydrated Notion context inventory and search hits.
3. Open the most relevant mounted `page.md` files to connect code changes to launch context.
4. Decide what is proven, what is unverified, and what should block launch.
5. Write the final Markdown memo to `OUT_DIR/report-body.md`.
