You are preparing a launch gate memo for Locality. Find the relevant notionproject context and recent code changes from `{{SANDBOX_HOME}}/workspace/locality`, decide what is actually proven, what is still unverified, and what should block launch. Produce a concise Markdown memo.

Do not use direct Notion API tools in this run. Do not create a new notion page or modify existing notion pages.

Write the final Markdown report to `{{AGENT_REPORT_PATH}}`.

Report format:

# Locality Launch Gate Memo

## Recommendation

## Evidence Reviewed

## Proven

## Unverified

## Launch Blockers

## Required Validation

The memo should be concise, specific, and grounded in evidence. If a claim cannot be verified from git, gh, or Locality context, say so.
