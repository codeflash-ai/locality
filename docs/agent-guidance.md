...
- For Gmail, outbound operations distinguish `draft/` for unsent drafts from `outbox/` for direct sends. Use `outbox/` only when the user explicitly asks to send now; otherwise use `draft/`.
