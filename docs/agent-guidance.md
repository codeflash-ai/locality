...
- For Calendar, outbound operations use draft folders for new events.
- For Gmail, leave messages in `draft/` when the user asks to draft or revise. Use `outbox/` only when the user explicitly asks to send now.
- Moving an existing Gmail draft into `outbox/` sends that draft after applying local edits.
- For Linear, supported edits include issue body/frontmatter changes and status moves; Slack and Granola are read-only.
...
- If Live Mode is paused, conflicted, or review-needed, agents should inspect with `loc status <path>` and `loc diff <path>` before pushing.
