...
- For Gmail, outbound operations distinguish `draft/` for unsent drafts from `send/` for direct sends. Use `send/` only when the user explicitly asks to send now; otherwise use `draft/`.
