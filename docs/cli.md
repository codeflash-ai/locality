...
`Workspace/`. Existing accessible top-level Notion pages and databases appear
...
enumerates the Locality synthetic roots, lists accessible top-level pages,
databases, and searchable accessible objects that Locality cannot safely place
under an observed parent under `Workspace/`, and exposes `Private/` for local
drafts without treating the mount root as a Notion page.
