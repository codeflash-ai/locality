...
The `loc` command is the single supported control surface for users and coding agents.
...
Remaining categories to assign before `loc push` applies remote mutations:
...
The default broker is `https://afs-oauth-broker.saurabh-b07.workers.dev`; override it with `--broker-url <url>`, `LOCALITY_NOTION_OAUTH_BROKER_URL`, or `LOCALITY_AUTH_BROKER_URL`. The default desktop callback is `http://localhost:8757/oauth/notion/callback`; override it with `--redirect-uri <uri>` or `LOCALITY_NOTION_OAUTH_REDIRECT_URI`. In the brokered product flow, the Notion public integration registers the broker HTTPS callback, such as `<LOCALITY_BROKER_PUBLIC_BASE_URL>/v1/oauth/notion/callback`; the broker redirects back to the desktop callback after Notion returns the authorization code.

`--direct-oauth` keeps the developer BYO OAuth path. In that mode the command reads `LOCALITY_NOTION_OAUTH_CLIENT_ID` and `LOCALITY_NOTION_OAUTH_CLIENT_SECRET` (or `NOTION_OAUTH_CLIENT_ID` / `NOTION_OAUTH_CLIENT_SECRET`) and exchanges directly with Notion. Direct OAuth stores the user-supplied client secret in the credential store so refresh can work, and should not be the default product path.
...
Google Docs mounts use Google Docs document access plus Drive `drive.file` and Drive `drive.metadata` access. Pull enumerates Google Docs and Drive folders under the configured workspace folder, including Docs manually added inside that workspace folder. Drive metadata writes are used for mounted-file rename, move, and trash workflows. Non-Google-Docs Drive files are ignored by this connector in V1. For Google OAuth verification, keep the submitted Google Docs API scopes aligned with `connectors/oauth-verification/google-docs.json`; do not submit full Drive, Drive readonly, Drive metadata readonly, or Docs readonly scopes.
...
Google Calendar OAuth uses `openid`, `email`, `profile`, and `https://www.googleapis.com/auth/calendar.events.owned`. The connector is primary-calendar-only: it pulls existing primary-calendar events and creates reviewed draft events on that primary calendar. For Google OAuth verification, keep the submitted Google Calendar API scopes aligned with `connectors/oauth-verification/google-calendar.json`; do not submit full calendar, all-calendar events, readonly-only, app-created only, freebusy, or settings scopes.

`loc connect gmail [--name <id>]` uses the Locality OAuth broker flow for Gmail. The default connection ID is `gmail-default` and the default OAuth profile is `gmail-oauth-default`. The default broker is `https://afs-oauth-broker.saurabh-b07.workers.dev`; override it with `--broker-url <url>`, `LOCALITY_GMAIL_OAUTH_BROKER_URL`, or `LOCALITY_AUTH_BROKER_URL`. The default callback is `http://localhost:8757/oauth/gmail/callback`; override it with `--redirect-uri <uri>` or `LOCALITY_GMAIL_OAUTH_REDIRECT_URI`. The same shared Google OAuth client is used for Gmail, Google Docs, and Google Calendar, so the client and broker allowlists should include all Google connector callbacks.

Gmail OAuth uses `openid`, `email`, `profile`, `https://www.googleapis.com/auth/gmail.readonly`, and `https://www.googleapis.com/auth/gmail.compose`. No broader Gmail account scope is required. For Google OAuth verification, keep the submitted Gmail API scopes aligned with `connectors/oauth-verification/gmail.json`; do not submit `https://mail.google.com/`, `gmail.modify`, `gmail.insert`, Gmail add-on current-message scopes, or other Gmail scopes the runtime OAuth profile does not request.

`loc mount gmail <path>` registers a Gmail mount. If `--connection` is omitted, the daemon resolves the mount through the only active Gmail connection at runtime; with multiple active Gmail connections, pass `--connection <id>`. When `--mount-id` is omitted, Locality uses `gmail-main` when available. Gmail mounts project `inbox/`, `sent/`, `draft/`, and `outbox/` folders. `inbox/` and `sent/` are read-only. `draft/` contains remote Gmail drafts and local draft creates; editing a remote draft and pushing updates the Gmail draft. `outbox/` is local-only send staging for reviewed direct sends.

Gmail mount options:

- `--after YYYY-MM-DD --before YYYY-MM-DD`: persist a Gmail date window for
  inbox, sent, and draft enumeration. The flags must be used together.
...
reported as explicit suggested commands, for example `loc connect notion`,
`loc daemon start`, or `loc file-provider start <mount>`.
  ...
  "findings": [
    {
      ...
      "mount_id": "notion-main",
      "message": "Virtual mount `notion-main` needs localityd running.",
      "suggested_command": "loc daemon start"
    }
  ],
  "suggested_commands": ["loc daemon start"]
}
```
...
```bash
loc search roadmap
loc search "Initial Idea"
loc search https://app.notion.com/p/codeflash/Initial-Idea-37b3ac0ebb88802cbcf4d53c9cfc4972
loc search roadmap --connector notion --limit 5 --json
```
...
`.locality-pack.yaml` plus Markdown templates, workflows, policies, and output
...
`loc templates new <pack> <path>` requires an empty target directory by default.
...
`loc templates apply <pack> <template> --to <dir> [--title <title>]` writes a
...
`loc diff <file>` and runs `loc push <file> -y`. This gives templates a safe
...
`loc file-provider unregister <mount>` unregisters the current platform's
...
contains remote Gmail drafts and local draft creates. Editing a remote draft and
pushing updates the Gmail draft. `outbox/` is not pulled from remote history; it
is a local-only reviewed direct-send staging folder, and successful pushes from
`outbox/` reconcile to `sent/`.
...
If the daemon socket is unavailable and `LOCALITY_DAEMON_DISABLE` is not set, pull/push print a stderr hint and continue directly:
...
`loc daemon start` starts `localityd` as a background daemon. On macOS, the default
manager is a per-user LaunchAgent at
`~/Library/LaunchAgents/ai.codeflash.locality.localityd.plist`, with stdout/stderr under
`~/.loc/logs/`. The LaunchAgent uses `RunAtLoad` and `KeepAlive`, so the daemon
...
`~/.loc/localityd.pid`; session mode inherits the current shell environment but does
not survive logout.
...
`--localityd-bin <path>` points the manager at a specific daemon binary. For
...
keychain-backed `loc connect` is available.

## Initial `loc info --json` Shape

`loc info [path]` explains the local source-of-record context for one path using only the SQLite state store. It defaults to the current working directory, resolves the containing mount, identifies the exact or nearest projected entity, reports immediate child counts, and includes pending/failed journal counts for that local context. It does not read or hydrate file bodies and does not call remote connectors.
...
## Initial `loc status --json` Shape

`loc status [path]` inspects Local Tree state and the latest Remote Tree metadata
...
view. Bare `loc status` does not scan the whole File Provider projection.

The production state directory defaults to `~/.loc`; `LOCALITY_STATE_DIR` is a developer/test override for isolated runs. When no path is supplied, `loc status` first checks the current working directory: inside a mount it scopes to that subtree, and outside all mounts it reports every registered mount in the active state directory.
...
- `checking_freshness`: Locality has Local Tree activity and is checking Remote Tree metadata;
...
or prints a clean line when every tracked entry is safe. If Locality is only checking
...
## Initial `loc inspect --json` Shape

`loc inspect <path>` is an explicit Remote Tree change explanation barrier for one
hydrated page. Unlike `loc status`, it is allowed to call the connector. It
compares:
...
`replace_block` operations. For Notion, that means Locality creates the replacement
...
Notion public API does not directly reposition existing blocks, so Locality applies
...
The JSON report has the same validation, plan, degradation, guardrail, and stage fields as `loc diff`. Its `action` is one of:
...
For Gmail, `loc push` supports new Markdown files directly under `draft/` or
`outbox/`, editing existing remote drafts under `draft/`, and moving existing
remote drafts from `draft/` to `outbox/`. Push from `draft/` creates or updates
an unsent Gmail draft. Push from `outbox/` directly sends the message, and a
moved remote draft is sent after applying local edits. Use `outbox/` only after
review when the user intends to send now. Gmail outbound files require `to`
frontmatter and either `subject` or `title`; `cc` and `bcc` are optional.
Outbound attachments remain unsupported. Nested outbound paths and edits or
deletes in `inbox/` and `sent/` are rejected.
...
`loc restore <path> [--force] [--json]` is a local recovery command. It resolves the path to a mounted entity, loads the Synced Tree shadow, rewrites the file atomically from canonical frontmatter plus the shadow body, refreshes the entity content hash, and marks hydration back to `hydrated`. It removes inline conflict markers by overwriting the file from shadow. It does not call Notion and does not delete failed journals, so `loc log` remains an audit trail.

`loc restore` refuses legacy conflicted entities unless `--force` is supplied.
...
`loc daemon status [--json]` checks the configured daemon control endpoint and,
...
`loc undo <push-id>` reads one journal entry and returns an undo decision. The current safe behavior is:
