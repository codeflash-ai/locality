...
- Notion reverse apply supports block content and property restore, archiving journaled created blocks/entities, restoring archived entities when a complete identity preimage is available, and restoring archived blocks in place by clearing `in_trash` so their identity and hierarchy remain intact;
