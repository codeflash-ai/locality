...
| E2E-014 | Supported rich block edits push and verify: paragraph, headings 1-4, bullet, number, todo, quote, callout, code, divider, equation, table, bookmark, embed, media URL/caption, block-type replacement, safe directive block moves, read-only `link_to_page`/database line moves, unsafe `link_to_page`/database retarget blocking, unsafe `link_preview` edit/move/delete blocking, table trailing-row delete, unsafe table shape and row-identity blocking, and unsafe child-page link edit/move/delete blocking. | Covered live | Notion apply/render tests; `crates/afs-cli/tests/e2e_push_workflow.rs::live_block_type_replace_pushes_and_reconciles_notion`; `crates/afs-cli/tests/e2e_push_workflow.rs::live_directive_block_move_pushes_and_reconciles_notion`; `crates/afs-cli/tests/e2e_push_workflow.rs::live_link_to_page_line_move_preserves_notion_block_type`; `crates/afs-cli/tests/e2e_push_workflow.rs::live_link_to_database_line_move_preserves_notion_block_type`; `crates/afs-cli/tests/e2e_push_workflow.rs::live_link_to_page_retarget_blocks_before_journaled_apply`; `crates/afs-cli/tests/e2e_push_workflow.rs::live_link_to_database_retarget_blocks_before_journaled_apply`; `crates/afs-cli/tests/e2e_push_workflow.rs::live_paragraph_notion_link_labeled_like_link_to_page_can_be_edited`; `crates/afs-cli/tests/e2e_push_workflow.rs::live_table_trailing_row_delete_pushes_and_reconciles`; `crates/afs-cli/tests/e2e_push_workflow.rs::live_table_width_change_blocks_before_journaled_apply`; `crates/afs-cli/tests/e2e_push_workflow.rs::live_table_middle_row_delete_blocks_before_journaled_apply`; `crates/afs-cli/tests/e2e_push_workflow.rs::live_child_page_link_move_blocks_before_journaled_apply`; `crates/afs-cli/tests/e2e_push_workflow.rs::live_child_page_link_delete_blocks_before_journaled_apply`. | Covered for representative edits, replacement, a childless directive move, rendered `link_to_page` and database link moves, `link_to_page` and database retarget guardrails, ordinary paragraph Notion links whose labels look like rendered `link_to_page` blocks, local `link_preview` edit/move/delete guardrails, supported table trailing-row delete, table width/header-mode and detected middle-row-delete pre-apply guardrails, and child-page link edit/move/delete guardrails; not every color/layout option. |
...
| E2E-027 | Push success journals the operation and enables future history/undo surfaces where possible. | Covered live | `crates/afs-cli/tests/history.rs`; `crates/afs-cli/tests/e2e_push_workflow.rs::live_push_log_and_undo_restores_remote_content`; `crates/afs-cli/tests/e2e_push_workflow.rs::live_directive_block_move_undo_restores_remote_order`; `crates/afs-cli/tests/e2e_push_workflow.rs::live_undo_archive_restores_paragraph_link_without_link_to_page_promotion`; `crates/afs-cli/tests/e2e_push_workflow.rs::live_undo_child_page_create_archives_remote_page`. | Covered for a reversible block update, a Notion copy+archive directive move undo, archived paragraph restore without label-based `link_to_page` promotion, and archiving a created child page on undo; broader undo variants remain local-only. |
...
| `crates/afs-cli/tests/e2e_push_workflow.rs::live_link_to_database_retarget_blocks_before_journaled_apply` | Live mounted workflow, plain files | Retarget a rendered database link Markdown line, assert diff/push report structured validation instead of creating a journal, and verify the live Notion source blocks are unchanged. Covers the unsafe database-link retarget guardrail for E2E-014. |
| `crates/afs-cli/tests/e2e_push_workflow.rs::live_paragraph_notion_link_labeled_like_link_to_page_can_be_edited` | Live mounted workflow, plain files | Create a real paragraph containing a Notion URL link labeled `Linked page`, edit the link target through Markdown, and verify Notion keeps it as a paragraph block rather than tripping the rendered `link_to_page` guardrail. Covers the paragraph/link false-positive regression for E2E-014. |
...
| `crates/afs-cli/tests/e2e_push_workflow.rs::live_virtual_database_row_directory_delete_archives_remote_row` | Live virtual filesystem core | Hydrate a database row, record a virtual row-directory delete from the database directory, verify the pending archive status and `archive_entity` plan, push to Notion, and verify the row page is archived and the local mutation is reconciled. Covers the database-row variant of E2E-020. |
...
| `crates/afs-cli/tests/e2e_push_workflow.rs::live_undo_archive_restores_paragraph_link_without_link_to_page_promotion` | Live mounted workflow, plain files | Delete a paragraph block whose visible Markdown is `[Linked page](...)`, push the archive, undo it, and verify Notion restores a paragraph block instead of promoting it to `link_to_page`. Covers archived-block undo fidelity for E2E-027. |
| `crates/afs-cli/tests/e2e_push_workflow.rs::live_undo_child_page_create_archives_remote_page` | Live mounted workflow, plain files | Create a child page from a local `page.md`, verify the create journal effect, undo the push with the Notion undo applier, and verify Notion archives the created child page. Covers created-entity undo for E2E-027. |
...
| `crates/afs-cli/tests/e2e_push_workflow.rs::live_local_image_media_edit_with_escaped_caption_uploads_and_reconciles` | Live mounted workflow, plain files | Overwrite a downloaded local image file, edit its caption to include escaped Markdown link-label characters, push the upload to Notion, and verify the reconciled local image bytes remain local. Covers escaped media-caption planning for E2E-010 and E2E-014. |
