...
| E2E-010 | File-like media references render in Markdown and downloadable media can be materialized into a local media directory for agent inspection. | Covered live | Media render/unit coverage. | Live test covers media download; broader media pruning/cache lifecycle is not covered. |
...
| E2E-014 | Supported rich block edits push and verify: paragraph, headings 1-4, bullet, number, todo, quote, callout, code, divider, equation, table, bookmark, embed, media URL/caption, and block-type replacement. | Covered live | Notion apply/render tests; `crates/afs-cli/tests/e2e_push_workflow.rs::live_block_type_replace_pushes_and_reconciles_notion`. | Covered for representative edits and replacement; not every color/layout option. |
...
| E2E-020 | Local creates/renames/deletes in virtual mounts surface as pending virtual mutations. | Covered live for virtual delete and Windows Cloud Files | `crates/afs-cli/tests/projection_contract.rs`; `tests/linux_fuse_smoke.sh`; virtual filesystem tests; `crates/afs-cli/tests/e2e_push_workflow.rs::live_virtual_page_directory_delete_archives_remote_child_page`. | Linux FUSE and macOS File Provider live create/rename/delete still need provider-backed Notion coverage. |
...
| E2E-027 | Push success journals the operation and enables future history/undo surfaces where possible. | Covered live | `crates/afs-cli/tests/history.rs`; `crates/afs-cli/tests/e2e_push_workflow.rs::live_push_log_and_undo_restores_remote_content`. | Covered for a reversible block update; broader undo operation variants remain local-only. |
...
mount, virtual page delete, drift preflight, remote fast-forward, and URL locate
coverage.
