...
only and do not contain `page.md`. When the connection exposes an owner user,
accessible top-level workspace pages created by that owner appear under
`Private/`; this is derived from the direct workspace parent plus owner creator.
Create private top-level pages under `Private/`; create children inside existing
page directories rather than directly under `Workspace/`.
