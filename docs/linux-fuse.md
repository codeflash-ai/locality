...
- `open`/`read`: hydrate online-only files through `virtual_fs_materialize`, then
  read bytes from the daemon-materialized canonical Markdown path. Existing
  read-only materialized files can be served directly from the cached path, so a
  slow background provider freshness job does not block warm read-only opens.
  File handles use FUSE direct I/O so a pre-hydration zero-length inode cannot
  make the kernel return EOF after `open` has materialized non-empty contents.
