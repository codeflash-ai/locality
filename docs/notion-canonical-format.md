...
The first writer supports block bodies whose Markdown shape maps to one Notion block or a guarded same-shape Notion table: paragraphs, headings, single list items, to-dos, quotes, code fences, dividers, display equations, existing tables with unchanged width/header mode/row count, existing bookmark/embed URL blocks, and existing URL-backed media blocks. Media edits write external URLs; uploads and appending new media blocks are deferred. It also parses the rich inline Markdown emitted by the renderer for bold, italic, strikethrough, underline, code, external links, equations, Notion page links, database links whose target ID matches a rendered database mention, and legacy `afs://` page links. Unchanged preimage mentions, such as date mentions, are preserved during block updates; unsupported inline shapes fail rather than being flattened silently.
