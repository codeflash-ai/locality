...
tmp_root="$(mktemp -d "${TMPDIR:-/tmp}/loc-linux-publish-validation.XXXXXX")"
...
if [[ "$(uname -s)" == "Linux" ]]; then
  ...
  touch \
    ...
    "${tmp_root}/deb/data/usr/bin/localityd" \
    "${tmp_root}/deb/data/usr/bin/locality-fuse"
  ...
  (
    cd "${tmp_root}/deb"
    ar r "${tmp_root}/loc.deb" debian-binary control.tar.gz data.tar.gz >/dev/null
  )

  validate_deb "${tmp_root}/loc.deb"
else
