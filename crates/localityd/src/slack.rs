...
fn connector_from_connection(
    ...
    Ok(SlackConnector::new(
        SlackConfig::new(token).with_settings(settings),
    ))
}
...
impl HydrationSource for SlackConnector {
    fn fetch_render(&self, request: &HydrationRequest) -> LocalityResult<HydratedEntity> {
        ...
        let document = locality_slack::render_slack_document(&bundle)?;
        ...
        Ok(HydratedEntity {
            document,
            shadow,
            remote_edited_at: Some(locality_slack::render::remote_version(&bundle)),
            assets: Vec::new(),
        })
    }
