...
use locality_store::{EntityRepository, MountRepository, SqliteStateStore};
use notify::event::{
...
pub trait FileWatcher {
    ...
    fn unwatch_mount(&mut self, _root: &Path) -> LocalityResult<()> {
        Ok(())
    }
...
impl NotifyFileWatcher {
    pub fn new(on_event: impl Fn(FileEvent) + Send + 'static) -> LocalityResult<Self> {
        let watcher = notify::recommended_watcher(move |event| match event {
            ...
            Err(error) => eprintln!("localityd watcher event failed: {error}"),
        })
...
impl FileWatcher for NotifyFileWatcher {
    ...
    fn unwatch_mount(&mut self, root: &Path) -> LocalityResult<()> {
...
impl PollingStubReadWatcher {
    pub fn new(
        ...
        let join = thread::Builder::new()
            .name("loc-stub-read-watcher".to_string())
            .spawn(move || {
            ...
            .map_err(|error| {
                LocalityError::Io(format!("failed to start stub read watcher: {error}"))
            })?;
...
fn run_stub_access_poll_loop(
    ...
    loop {
        ...
        {
            eprintln!("localityd stub read watcher failed: {error}");
        }
...
fn poll_stub_accesses(
    ...
    for mount in store.load_mounts().map_err(LocalityError::from)? {
...
fn watcher_error(error: notify::Error) -> LocalityError {
    LocalityError::Io(format!("file watcher failed: {error}"))
}
...
mod tests {
    ...
    fn temp_root(name: &str) -> PathBuf {
        let root =
            std::env::temp_dir().join(format!("loc-watcher-unit-{name}-{}", std::process::id()));
