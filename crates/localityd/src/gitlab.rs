...
use locality_gitlab::{
    GITLAB_CONNECTOR_ID, GitLabConfig, GitLabConnector, GitLabNativeBundle, render_gitlab_entity,
};
use locality_store::{
...
pub(crate) const GITLAB_CONNECT_COMMAND: &str = "loc connect gitlab --api-key-stdin";

pub fn resolve_gitlab_connector_for_mount<S>(
    store: &S,
    credentials: &dyn CredentialStore,
    mount: &MountConfig,
) -> Result<GitLabConnector, ConnectorResolveError>
where
    ...
    let connection = if let Some(connection_id) = &mount.connection_id {
        store
            ...
            .ok_or_else(|| ConnectorResolveError::MissingConnection {
                ...
                suggested_command: GITLAB_CONNECT_COMMAND.to_string(),
            })?
    } else {
        let active = active_connections(store)?;
        if active.len() != 1 {
            let message = if active.is_empty() {
                "missing GitLab connection; run `loc connect gitlab --api-key-stdin`".to_string()
            } else {
                "mount has no connection_id and multiple GitLab connections exist".to_string()
            };
    ...
    S: ConnectionRepository + ?Sized,
{
    Ok(store
        ...
        .into_iter()
        .filter(|connection| {
            connection.connector == GITLAB_CONNECTOR_ID
    ...
    S: ConnectorProfileRepository + ?Sized,
{
    ...
    if profile.status != "active"
        || profile.connector != GITLAB_CONNECTOR_ID
...
fn connector_from_connection(
    ...
    if connection.connector != GITLAB_CONNECTOR_ID || connection.auth_kind != "api_key" {
        return Err(ConnectorResolveError::AuthRequired {
            connection_id: connection.connection_id.0.clone(),
            message: Some("GitLab connections require a personal access token".to_string()),
            suggested_command: GITLAB_CONNECT_COMMAND.to_string(),
        });
    }
    ...
    Ok(GitLabConnector::new(GitLabConfig::new(token)))
}
...
impl SourcePushValidator for GitLabConnector {}
impl SourceAdapter for GitLabConnector {}

impl HydrationSource for GitLabConnector {
    fn fetch_render(&self, request: &HydrationRequest) -> LocalityResult<HydratedEntity> {
        ...
        let bundle = serde_json::from_slice::<GitLabNativeBundle>(&native.raw)
            .map_err(|error| LocalityError::Io(format!("GitLab native decode failed: {error}")))?;
        let document = render_gitlab_entity(&bundle)?;
...
pub fn validate_gitlab_frontmatter(
