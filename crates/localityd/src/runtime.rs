...
impl RuntimeState {
    ...
    fn handle_completion(&mut self, completion: JobCompletion) {
        ...
        match completion {
            ...
            JobCompletion::Hydration {
                ...
                Ok(outcome) => {
                    self.cancel_hydration_target(&request.mount_id, &request.remote_id);
    ...
    fn cancel_prefetch_target(&mut self, mount_id: &MountId, remote_id: &RemoteId) {
        ...
        self.deferred_hydration.retain(|request| {
            request.mount_id != *mount_id
                || request.remote_id != *remote_id
                || request.reason != HydrationReason::Prefetch
        });
        if self.deferred_hydration.is_empty() {
            self.next_hydration_retry = None;
        }
        let target_is_still_queued = self.hydration.contains_target(mount_id, remote_id)
            || self
                .deferred_hydration
                .iter()
                .any(|request| request.mount_id == *mount_id && request.remote_id == *remote_id);
    ...
    fn prune_obsolete_prefetches(&mut self) {
        ...
        requests.extend(self.deferred_hydration.iter().cloned());
...
fn file_provider_children_response_after_refresh(
    ...
    let children =
        runner.run_virtual_fs_children(state_root, mount_id.clone(), container_identifier.clone());
    if refresh_result.is_ok() || children.ok {
        return children;
    }

    let error = refresh_result
        .as_ref()
        .expect_err("refresh_result should be an error");
    DaemonResponse::error(locality_error_code(error), error.to_string())
}
...
mod tests {
    ...
    fn reconciled_gmail_send_response() -> DaemonResponse {
        ...
        let operation_id = PushOperationId("create-gmail-outbox".to_string());
        let push_id = PushId("push-gmail-outbox".to_string());
        ...
        DaemonResponse::ok(PushJobReport {
            target_path: "/tmp/gmail-main/outbox/test-message.md".into(),
            mount_id: MountId::new("gmail-main"),
            entity_id: RemoteId::new("local:gmail-outbox"),
            pipeline,
            readable_diff: None,
    ...
    fn save_reconciled_gmail_send_journal(store: &mut InMemoryStateStore) {
        store
            .append_journal(
                JournalEntry::new(
                    PushId("push-gmail-outbox".to_string()),
                ...
                .with_metadata(
                    JournalMetadata::default().with_local_projection_items(vec![
                        JournalLocalProjectionItem {
                            operation_index: 0,
                            local_id: "local:gmail-outbox".to_string(),
                        },
                    ]),
                ),
            )
        ...
        assert_eq!(
            ...
            vec![ProjectionRefreshRequest {
                ...
                container_identifiers: vec![
                    "gmail-folder:outbox".to_string(),
                    "gmail-folder:sent".to_string(),
                ],
                local_item_identifiers: vec!["local:gmail-outbox".to_string()],
            }]
        );
    }
    ...
    fn gmail_file_provider_refresh_removes_local_send_before_signaling_containers() {
        ...
        .expect("remove outbox item and signal containers");
