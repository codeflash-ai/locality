...
use localityd::execution::{AdvanceScheduledPullJob, DaemonExecutor};
...
#[test]
fn scheduled_pull_keeps_entity_path_when_reserved_root_final_destination_exists() {
    let root = temp_root("scheduled-pull-reserved-root-final-destination-exists");
    let mount_id = MountId::new("notion-main");
    let remote_id = RemoteId::new("page-1");
    let mount = MountConfig::new(mount_id.clone(), "notion", root.clone());
    ...
    std::fs::create_dir_all(root.join("private")).expect("old container");
    std::fs::write(root.join(&old_path), "old body").expect("old page");
    std::fs::create_dir_all(root.join("Workspace/private")).expect("destination container");
    std::fs::write(root.join(&new_path), "unrelated body").expect("unrelated destination page");
    let mut source = FakeScheduledPullSource::default();
    source.insert_entries(
        &mount_id,
        vec![
    ...
    assert!(result.is_err());
    assert_eq!(
        std::fs::read_to_string(root.join(&old_path)).expect("old page"),
        "old body"
    );
    assert_eq!(
        std::fs::read_to_string(root.join(&new_path)).expect("unrelated destination page"),
        "unrelated body"
    );
    let entity = store
        .get_entity(&mount_id, &remote_id)
...
#[test]
fn scheduled_pull_keeps_reserved_root_disk_when_unrelated_projection_fails_before_entity_save() {
    let root = temp_root("scheduled-pull-reserved-root-unrelated-failure");
    let mount_id = MountId::new("notion-main");
    let remote_id = RemoteId::new("page-1");
    let mount = MountConfig::new(mount_id.clone(), "notion", root.clone());
    let old_path = PathBuf::from("private/page.md");
    ...
    source.insert_entries(
        ...
        vec![
            directory_entry(&mount_id, "notion-root:private", "Private", "Private"),
            directory_entry(&mount_id, "notion-root:workspace", "Workspace", "Workspace"),
            database_entry(&mount_id, "blocked-db", "Blocked", "Blocked/Tasks"),
            page_entry(
                &mount_id,
                "page-1",
                "private",
...
#[test]
fn scheduled_pull_saves_reserved_root_descendant_paths_when_later_entry_fails() {
    let root = temp_root("scheduled-pull-reserved-root-descendant-later-failure");
    let mount_id = MountId::new("notion-main");
...
fn scheduled_pull_keeps_reserved_root_disk_when_final_destination_is_created_before_repair() {
    ...
    source.insert_entries(
        ...
        vec![
            directory_entry(&mount_id, "notion-root:private", "Private", "Private"),
            directory_entry(&mount_id, "notion-root:workspace", "Workspace", "Workspace"),
            database_entry(&mount_id, "occupied-db", "private", "Workspace/private"),
            page_entry(
                &mount_id,
                "page-1",
                "private",
...
#[test]
fn scheduled_pull_does_not_move_unrelated_reserved_root_for_normal_rename() {
    let root = temp_root("scheduled-pull-normal-rename-reserved-root");
    let mount_id = MountId::new("notion-main");
    let remote_id = RemoteId::new("page-1");
    let mount = MountConfig::new(mount_id.clone(), "notion", root.clone());
    let old_path = PathBuf::from("Old/page.md");
    ...
    std::fs::create_dir_all(root.join("Old")).expect("old container");
    std::fs::write(root.join(&old_path), "old body").expect("old page");
    std::fs::create_dir_all(root.join("private")).expect("unrelated container");
    std::fs::write(root.join("private/page.md"), "unrelated body").expect("unrelated page");
    let mut source = FakeScheduledPullSource::default();
    source.insert_entries(
        &mount_id,
        vec![
    ...
    assert_eq!(
        std::fs::read_to_string(root.join(&new_path)).expect("moved page"),
        "old body"
    );
    assert_eq!(
        std::fs::read_to_string(root.join("private/page.md")).expect("unrelated page"),
        "unrelated body"
    );
    let entity = store
        .get_entity(&mount_id, &remote_id)
