...
use localityd::execution::{DaemonExecutor, HydrationDrainJob, HydrationRequestJob};
...
use localityd::watcher::{FileEvent, FileEventKind, FileWatcher};
...
fn supervisor_start_registers_mount_roots_with_watcher() {
    ...
    assert_eq!(
        ...
        vec![PathBuf::from("/tmp/loc/notion")]
    );
...
fn supervisor_start_skips_virtual_projection_mount_roots() {
    ...
    assert_eq!(
        supervisor.watcher().watched,
        vec![PathBuf::from("/tmp/loc/notion-plain")]
    );
...
fn virtual_projection_file_event_is_ignored_by_host_watcher_path() {
    ...
    let report = supervisor
        .execute_file_event(FileEvent {
            path: PathBuf::from("/tmp/loc/notion-fuse/Roadmap.md"),
...
fn read_event_on_stub_queues_hydration() {
    ...
    let report = supervisor
        .execute_file_event(read_event("/tmp/loc/notion/Roadmap.md"))
        .expect("handle read");
    ...
    assert_eq!(request.path, PathBuf::from("/tmp/loc/notion/Roadmap.md"));
...
fn write_event_on_hydrated_file_marks_entity_dirty() {
    ...
    let report = supervisor
        .execute_file_event(FileEvent {
            path: PathBuf::from("/tmp/loc/notion/Roadmap.md"),
...
fn supervisor_drains_queued_hydration_through_source() {
    let _ = std::fs::remove_file("/tmp/loc/notion/Roadmap.md");
    let mut supervisor = supervisor_with_entity(HydrationState::Stub);
    supervisor.start().expect("start supervisor");
    supervisor
        .execute_file_event(read_event("/tmp/loc/notion/Roadmap.md"))
    ...
    let contents = std::fs::read_to_string("/tmp/loc/notion/Roadmap.md")
...
fn supervisor_hydrates_single_request_through_daemon_job() {
    let root =
        std::env::temp_dir().join(format!("loc-supervisor-hydrate-job-{}", std::process::id()));
    ...
    assert_eq!(
        report.outcome,
        localityd::hydration::HydrationOutcome::Hydrated
    );
...
fn supervisor_with_entity(
    ...
    supervisor_with_entity_at(PathBuf::from("/tmp/loc/notion"), hydration)
}
...
impl FileWatcher for RecordingWatcher {
    fn watch_mount(&mut self, root: PathBuf) -> locality_core::LocalityResult<()> {
