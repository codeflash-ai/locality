...
pub(crate) fn bootstrap_google_docs_portable(
    ...
    let root_id = portable_scope_root_id(connector)?;
    if request.scope.root_remote_ids.as_slice() != [root_id.clone()] {
    ...
    let mut changes =
        portable_google_docs_changes(&request.source_connection_id, &root_id, entries)?;
    ...
    Ok(PortableChangeBatch {
        changes,
        next_checkpoint: google_docs_portable_checkpoint(&root_id),
...
pub(crate) fn render_google_docs_portable(
    ...
    let root_id = portable_scope_root_id(connector)?;
    if portable.root_remote_id != root_id {
        return Err(LocalityError::InvalidState(format!(
    ...
    Ok(PortableRenderResult {
        canonical: PortableContentArtifact {
...
fn portable_scope_root_id(connector: &GoogleDocsConnector) -> LocalityResult<RemoteId> {
    let root_id = connector.portable_scope_root().cloned().ok_or_else(|| {
        LocalityError::InvalidState(
            "google docs portable export is missing a configured scope root".to_string(),
        )
    })?;
...
fn ensure_drive_file_within_root(
    ...
    while let Some(parent_id) = parents.pop() {
        ...
        let parent = drive.get_file(&parent_id).map_err(|_| {
            LocalityError::Guardrail(format!(
