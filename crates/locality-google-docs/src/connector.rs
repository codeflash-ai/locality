...
use locality_connector::{
    ...
    ListChildrenResult, NativeEntity, ObserveRequest, PORTABLE_SCOPE_ROOT_RELATIONSHIP,
    ...
    PortableRenderRequest, PortableRenderResult, PortableSourceChange,
};
...
impl Connector for GoogleDocsConnector {
    ...
    fn bootstrap_portable(
        ...
        let configured_root_id = self.portable_workspace_root_id()?;
        ...
        let mut changes = portable_google_docs_changes(
        ...
        let completeness = if complete {
            ...
            PortableCompleteness::incomplete(PortableIncompleteReason::ConnectorLimitation {
                code: "google_docs_bootstrap_max_changes_exceeded".to_string(),
                remote_id: Some(configured_root_id.clone()),
            })
        };
    ...
    fn fetch_portable(&self, request: PortableFetchRequest) -> LocalityResult<PortableFetchResult> {
        let root_id = self.portable_workspace_root_id()?;
        let drive_file = self.drive.get_file(request.remote_id.as_str())?;
        ...
        ensure_drive_file_within_root(self.drive.as_ref(), &drive_file, &root_id)?;
        ...
        let provider_version =
            combined_remote_version(&drive_file, document.revision_id.as_deref());
    ...
    fn render_portable(
        ...
        let portable = serde_json::from_slice::<GoogleDocsPortableNativeBundle>(
            &request.native.raw,
        )
        .map_err(|error| {
            LocalityError::Io(format!(
...
impl GoogleDocsConnector {
    fn portable_workspace_root_id(&self) -> LocalityResult<RemoteId> {
        let root_id = self.config.workspace_folder_id.clone().ok_or_else(|| {
            LocalityError::InvalidState(
                "google docs portable export is missing workspace folder id".to_string(),
            )
        })?;
        let root = self.drive.get_file(root_id.as_str())?;
    ...
    fn portable_workspace_folder_bootstraps_fetches_and_renders_document() {
        ...
        let connector = GoogleDocsConnector::with_apis(
            GoogleDocsConfig::new("token")
                .with_workspace_folder_id(RemoteId::new("drive-folder-1")),
            drive,
            docs,
        );
