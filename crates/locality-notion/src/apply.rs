...
) -> LocalityResult<ApplyUndoResult> {
...
) -> LocalityResult<Vec<NotionPageBundle>> {
    affected_entities
        .iter()
...
) -> LocalityResult<Option<(RemoteId, Value)>> {
...
fn existing_block_append_child(block: &BlockDto) -> LocalityResult<Value> {
...
fn existing_block_payload(block: &BlockDto) -> LocalityResult<Value> {
    match block.kind.as_str() {
        ...
        "child_page" | "child_database" | "link_preview" => Err(LocalityError::Unsupported(
            "moving this Notion block type by copy is not supported",
        )),
        _ => Err(LocalityError::Unsupported("moving this Notion block type")),
    }
}

fn file_payload(block: &BlockDto, payload: Option<&FileBlockDto>) -> LocalityResult<Value> {
...
fn link_to_page_payload(block: &BlockDto) -> LocalityResult<Value> {
    let payload = block
        .link_to_page
        .as_ref()
        .ok_or_else(|| missing_block_payload(block))?;
...
fn missing_block_payload(block: &BlockDto) -> LocalityError {
    LocalityError::InvalidState(format!(
    ...
    for (key, value) in properties {
        let (notion_key, property) = if key == "title" {
            ...
            let property = page.properties.get(key).ok_or_else(|| {
                LocalityError::Validation(vec![property_issue(
                    key,
...
fn create_page_children(body: &str) -> LocalityResult<Vec<Value>> {
    ...
    blocks
        ...
        .map(|block| {
            if block.is_directive() {
                return Err(LocalityError::Unsupported(
                    "creating Notion pages with Locality directive blocks",
                ));
            }
...
fn title_property(page: &PageDto) -> LocalityResult<(&str, &PagePropertyDto)> {
    page.properties
        .iter()
        .find(|(_, property)| property.kind == "title")
...
fn property_value_for_kind(kind: &str, value: &PropertyValue, key: &str) -> LocalityResult<Value> {
    match kind {
        ...
        _ => Err(LocalityError::Unsupported(
            "updating this Notion property type",
        )),
    }
}
...
fn number_property(value: &PropertyValue, key: &str) -> LocalityResult<Value> {
    match value {
...
fn option_property(kind: &str, value: &PropertyValue, key: &str) -> LocalityResult<Value> {
    match value {
...
fn multi_select_property(value: &PropertyValue, key: &str) -> LocalityResult<Value> {
    match value {
...
fn bool_property(value: &PropertyValue, key: &str) -> LocalityResult<Value> {
    match value {
...
fn date_property(value: &PropertyValue, key: &str) -> LocalityResult<Value> {
    match value {
...
fn nullable_string_property(kind: &str, value: &PropertyValue, key: &str) -> LocalityResult<Value> {
    match value {
        PropertyValue::Null => Ok(single_property(kind, Value::Null)),
        PropertyValue::String(value) if value.trim().is_empty() => {
            Ok(single_property(kind, Value::Null))
...
fn files_property(value: &PropertyValue, key: &str) -> LocalityResult<Value> {
...
fn external_file_property_value(entry: &str, key: &str) -> LocalityResult<Value> {
    let (name, url) = parse_external_file_entry(entry);
    if url.trim().is_empty() || !valid_url(url) {
        return Err(LocalityError::Validation(vec![property_issue(
            key,
...
fn people_property(value: &PropertyValue, key: &str) -> LocalityResult<Value> {
    let entries = match value {
        PropertyValue::Null => Vec::new(),
        PropertyValue::String(value) if value.trim().is_empty() => Vec::new(),
        PropertyValue::String(value) => vec![value.as_str()],
...
fn people_property_value(entry: &str, key: &str) -> LocalityResult<Value> {
...
fn relation_property(value: &PropertyValue, key: &str) -> LocalityResult<Value> {
    let entries = match value {
        PropertyValue::Null => Vec::new(),
        PropertyValue::String(value) if value.trim().is_empty() => Vec::new(),
        PropertyValue::String(value) => vec![value.as_str()],
...
fn relation_property_value(entry: &str, key: &str) -> LocalityResult<Value> {
    let id = parse_named_id_entry(entry).trim();
    if !valid_notion_id(id) {
        return Err(LocalityError::Validation(vec![property_issue(
            key,
...
fn property_type_error(key: &str, expected: &str) -> LocalityError {
    LocalityError::Validation(vec![property_issue(
        key,
...
) -> locality_core::validation::ValidationIssue {
...
fn ensure_update_supported(current: &BlockDto, patch: &NotionBlockPatch) -> LocalityResult<()> {
    if current.kind != patch.kind {
        return Err(LocalityError::Unsupported("changing Notion block type"));
    }

    Ok(())
}

fn current_block_rich_text(block: &BlockDto) -> LocalityResult<Option<&[RichTextDto]>> {
...
fn parse_markdown_table(
...
) -> LocalityResult<ParsedMarkdownTable> {
    let lines = markdown
        .lines()
    ...
    if lines.len() < 2 {
        return Err(LocalityError::Unsupported(
            "writing malformed Notion tables",
        ));
    }
...
fn parse_supported_block(
    ...
    preimage: Option<&[RichTextDto]>,
) -> LocalityResult<NotionBlockPatch> {
    let trimmed = markdown.trim_end_matches('\n');

    if trimmed.trim().is_empty() {
        return Err(LocalityError::Unsupported("empty Notion block writes"));
    }
    ...
    if let Some((level, text)) = parse_heading(trimmed) {
        let kind = match level {
            ...
            3 => "heading_3",
            4 => "heading_4",
            _ => return Err(LocalityError::Unsupported("Notion heading levels above 4")),
        };
        return Ok(NotionBlockPatch::new(
            kind,
    ...
    if looks_like_markdown_table(trimmed) {
        return Err(LocalityError::Unsupported("writing Notion tables"));
    }
...
fn parse_append_block(
    ...
    if let Some((caption, href, image_syntax)) = parse_local_media_markdown(trimmed) {
        ...
        let Some(local_path) =
            ...
            if image_syntax || looks_like_media_href(&href) {
                return Err(LocalityError::Unsupported(
                    "appended local media blocks must reference .loc/media under the projection output root",
                ));
            }
            return parse_supported_block(markdown, None, None);
        };
...
fn parse_new_table_append_child(markdown: &str) -> LocalityResult<Value> {
    ...
    if rows.is_empty() {
        return Err(LocalityError::Unsupported("creating empty Notion tables"));
    }
...
impl RichTextWritePart {
    ...
    fn apply_link(&mut self, href: &str) -> LocalityResult<()> {
        match self {
            Self::Text { link, .. } => {
            ...
            _ => Err(LocalityError::Unsupported("links on non-text rich spans")),
        }
    }
...
fn parse_rich_text_markdown(
    content: &str,
    preimage: Option<&[RichTextDto]>,
) -> LocalityResult<Vec<RichTextWritePart>> {
...
impl InlineParser<'_> {
    fn parse_until(&mut self, closing: Option<&str>) -> LocalityResult<Vec<RichTextWritePart>> {
    ...
    fn parse_special(
        &self,
        index: usize,
    ) -> LocalityResult<Option<(Vec<RichTextWritePart>, usize)>> {
...
fn parse_date_mention_args(
    input: &str,
) -> LocalityResult<(String, Option<String>, Option<String>)> {
...
fn invalid_date_mention_syntax() -> LocalityError {
    LocalityError::Unsupported(
        "date mention syntax; use @date(YYYY-MM-DD) or @date(YYYY-MM-DD to YYYY-MM-DD)",
    )
}

fn parse_user_mention_arg(input: &str) -> LocalityResult<String> {
...
fn parse_page_mention_arg(input: &str) -> LocalityResult<String> {
    let id = parse_named_id_entry(input).trim();
    if valid_notion_id(id) {
        Ok(id.to_string())
    } else {
...
fn parse_database_mention_arg(input: &str) -> LocalityResult<String> {
    let id = parse_named_id_entry(input).trim();
    if valid_notion_id(id) {
        Ok(id.to_string())
    } else {
...
fn looks_like_media_href(href: &str) -> bool {
    ...
    normalized == ".loc/media"
        || normalized.starts_with(".loc/media/")
        || normalized.contains("/.loc/media/")
}

fn read_local_media(local_root: &Path, local_path: &Path) -> LocalityResult<Vec<u8>> {
...
fn media_kind(kind: &str) -> LocalityResult<&'static str> {
    match kind {
        "image" => Ok("image"),
        "video" => Ok("video"),
        "file" => Ok("file"),
...
fn notion_page_id_from_href(href: &str) -> Option<String> {
    if let Some(id) = href.strip_prefix("loc://") {
    ...
    match mention.kind.as_str() {
        ...
        "date" => {
            let date = mention.date.as_ref().ok_or_else(|| {
                LocalityError::InvalidState("notion date mention had no date payload".to_string())
            })?;
            let mut value = json!({
                "type": "date",
                "date": {
        ...
        _ => Err(LocalityError::Unsupported(
            "preserving this Notion mention type",
        )),
    }
}
