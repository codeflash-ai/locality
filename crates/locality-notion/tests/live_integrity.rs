...
use locality_connector::{ApplyPlanRequest, Connector, FetchRequest};
...
fn live_page_read_edit_write_verify_integrity_with_media_download() {
    ...
    cleanup.create_page(&page.id, "Locality nested child page", Vec::new());
    ...
    let plan = PushPlan::new(
        ...
        vec![
            ...
            PushOperation::AppendBlock {
                ...
                content: "Appended from Locality live integrity.".to_string(),
            },
        ],
    );
    ...
    assert!(
        verified_render
            .document
            .body
            .contains("Appended from Locality live integrity.")
    );
    assert!(
        verified_render
            .document
...
fn live_database_row_property_create_edit_verify_integrity() {
    ...
    let scratch = cleanup.create_page(
        &env.parent_page_id,
        &format!("Locality live database scratch {}", unique_suffix()),
        Vec::new(),
    );
    let related_database = cleanup.create_database(
        &scratch.id,
        &format!("Locality live related rows {}", unique_suffix()),
    );
    ...
    let related_row_initial = cleanup.create_database_row(
        &related_database,
        &format!("Locality live related initial {}", unique_suffix()),
        serde_json::Map::new(),
        Vec::new(),
    );
    let related_row_updated = cleanup.create_database_row(
        &related_database,
        &format!("Locality live related updated {}", unique_suffix()),
        serde_json::Map::new(),
        Vec::new(),
    );
    let database = cleanup.create_database_with_relation(
        &scratch.id,
        &format!("Locality live rows {}", unique_suffix()),
    ...
    let plan = PushPlan::new(
        ...
        vec![PushOperation::CreateEntity {
            ...
            properties: BTreeMap::from([
                ...
                (
                    "URL".to_string(),
                    PropertyValue::String("https://example.com/loc-live".to_string()),
                ),
                (
                    "Email".to_string(),
                    PropertyValue::String("locality@example.com".to_string()),
                ),
                (
                    "Phone".to_string(),
            ...
            source_path: "Rows/loc-created-row.md".into(),
        }],
    );
    ...
    let row_id = result
        ...
        .find_map(|effect| match effect {
            locality_core::journal::JournalApplyEffect::CreatedEntity { entity_id, .. } => {
    ...
    let rendered = connector
        .render_native_entity_for_path(&native, "Rows/loc-created-row.md")
        .expect("render created row");
    assert!(
        rendered
            .document
            .frontmatter
            .contains("title: \"Locality created row\"")
    );
    ...
    assert!(
        rendered
            .document
            .frontmatter
            .contains("\"URL\": \"https://example.com/loc-live\"")
    );
    assert!(
        rendered
            .document
    ...
    let update = PushPlan::new(
        ...
        vec![PushOperation::UpdateProperties {
            ...
            properties: BTreeMap::from([
                ...
                (
                    "URL".to_string(),
                    PropertyValue::String("https://example.com/loc-live-updated".to_string()),
                ),
                (
                    "Files".to_string(),
    ...
    let verified_render = connector
        .render_native_entity_for_path(&verified, "Rows/loc-created-row.md")
    ...
    assert!(
        verified_render
            .document
            .frontmatter
            .contains("\"URL\": \"https://example.com/loc-live-updated\"")
    );
    assert!(
        verified_render
            .document
...
impl LiveEnv {
    fn from_env() -> Self {
        ...
        let local_dir = std::env::var(LIVE_DIR_ENV)
            .map(PathBuf::from)
            .unwrap_or_else(|_| {
                std::env::temp_dir().join(format!("locality-notion-live-{}", unique_suffix()))
            });
...
fn apply_plan(connector: &NotionConnector, plan: &PushPlan) -> locality_connector::ApplyPlanResult {
