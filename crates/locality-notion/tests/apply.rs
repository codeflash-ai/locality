    ...
    let plan = PushPlan::new(
        ...
        vec![
            ...
            PushOperation::UpdateBlock {
                ...
                content: "\\::loc{id=literal type=paragraph} changed".to_string(),
            },
        ],
    );
    let push_id = PushId("push-1".to_string());
    ...
    assert_eq!(
        ...
        [
            ...
            WriteCall::Update {
                ...
                body: json!({
                    "paragraph": {
                        "rich_text": rich_text_json("::loc{id=literal type=paragraph} changed"),
                    },
                }),
            },
        ]
...
fn apply_decodes_escaped_literal_break_tags() {
    ...
    let plan = PushPlan::new(
        ...
        vec![PushOperation::UpdateBlock {
            block_id: RemoteId::new("paragraph-1"),
            content: "Literal \\<br> tag changed".to_string(),
        }],
    );
    let push_id = PushId("push-1".to_string());
    let operation_ids = operation_ids(&push_id, &plan);
    ...
    assert_eq!(
        ...
        [WriteCall::Update {
            ...
            body: json!({
                "paragraph": {
                    "rich_text": rich_text_json("Literal <br> tag changed"),
                },
            }),
        }]
    );
...
#[test]
fn apply_decodes_escaped_literal_underline_tags() {
    let api = Arc::new(RecordingNotionApi::with_paragraph_rich_text(
        "2026-06-10T00:00:00.000Z",
        rich_text("Original"),
    ));
    ...
    let plan = PushPlan::new(
        ...
        vec![PushOperation::UpdateBlock {
            block_id: RemoteId::new("paragraph-1"),
            content: "Literal \\<u>tag\\</u> changed".to_string(),
        }],
    );
    let push_id = PushId("push-1".to_string());
    let operation_ids = operation_ids(&push_id, &plan);
    ...
    assert_eq!(
        ...
        [WriteCall::Update {
            ...
            body: json!({
                "paragraph": {
                    "rich_text": rich_text_json("Literal <u>tag</u> changed"),
                },
            }),
        }]
    );
...
#[test]
fn apply_decodes_escaped_literal_equation_markers() {
    let api = Arc::new(RecordingNotionApi::with_paragraph_rich_text(
        "2026-06-10T00:00:00.000Z",
        rich_text("Original"),
    ));
    ...
    let plan = PushPlan::new(
        ...
        vec![PushOperation::UpdateBlock {
            block_id: RemoteId::new("paragraph-1"),
            content: "Literal \\$E=mc^2\\$ changed".to_string(),
        }],
    );
    let push_id = PushId("push-1".to_string());
    let operation_ids = operation_ids(&push_id, &plan);
    ...
    assert_eq!(
        ...
        [WriteCall::Update {
            ...
            body: json!({
                "paragraph": {
                    "rich_text": rich_text_json("Literal $E=mc^2$ changed"),
                },
            }),
        }]
    );
...
fn apply_decodes_escaped_literal_markdown_inline_markers() {
    ...
    let connector = NotionConnector::with_api(NotionConfig::default(), api.clone());
    let literal = "Literal **bold** _italic_ ~~strike~~ `code` [link](https://example.com) changed";
    let escaped = "Literal \\**bold\\** \\_italic\\_ \\~~strike\\~~ \\`code\\` \\[link](https://example.com) changed";
    let plan = PushPlan::new(
        vec![RemoteId::new("page-1")],
        vec![PushOperation::UpdateBlock {
            block_id: RemoteId::new("paragraph-1"),
...
fn apply_rejects_link_to_page_retargeting_before_api_mutation() {
    ...
    assert!(
        matches!(error, LocalityError::Unsupported(message) if message.contains("link_to_page")),
        "{error:?}"
    );
    assert!(api.writes.lock().expect("writes").is_empty());
}
...
fn apply_undo_restores_archived_directive_by_appending_replacement() {
    ...
    let undo_plan = UndoPlan {
        ...
        operations: vec![UndoOperation::RestoreArchivedBlock {
            ...
            parent_id: RemoteId::new("page-1"),
            after: Some(RemoteId::new("paragraph-1")),
            content: "::loc{id=toc-1 type=table_of_contents color=\"default\"}".to_string(),
            native_kind: None,
        }],
        unsupported: vec![],
        status: UndoPlanStatus::Complete,
...
fn apply_rejects_child_page_link_edits_before_api_mutation() {
    ...
    assert!(
        matches!(error, LocalityError::Unsupported(message) if message.contains("child_page")),
        "{error:?}"
    );
    assert!(api.writes.lock().expect("writes").is_empty());
}
...
fn apply_uploads_local_image_media_before_block_update() {
    ...
    let media_path = temp.path().join(".loc/media/Roadmap/image-1.png");
    ...
    let plan = PushPlan::new(
        ...
        vec![PushOperation::UpdateMedia {
            block_id: RemoteId::new("image-1"),
            local_path: ".loc/media/Roadmap/image-1.png".into(),
...
fn apply_uploads_local_file_like_media_before_block_update() {
    let temp = tempdir().expect("tempdir");
    let video_path = temp.path().join(".loc/media/Roadmap/cars.mp4");
    let pdf_path = temp.path().join(".loc/media/Roadmap/brief.pdf");
    let audio_path = temp.path().join(".loc/media/Roadmap/theme.mp3");
    let html_path = temp.path().join(".loc/media/Roadmap/index.html");
    ...
    let plan = PushPlan::new(
        ...
        vec![
            PushOperation::UpdateMedia {
                block_id: RemoteId::new("video-1"),
                local_path: ".loc/media/Roadmap/cars.mp4".into(),
                caption: "Updated video".to_string(),
            },
            PushOperation::UpdateMedia {
                block_id: RemoteId::new("pdf-1"),
                local_path: ".loc/media/Roadmap/brief.pdf".into(),
                caption: "Updated PDF".to_string(),
            },
            PushOperation::UpdateMedia {
                block_id: RemoteId::new("audio-1"),
                local_path: ".loc/media/Roadmap/theme.mp3".into(),
                caption: "Updated audio".to_string(),
            },
            PushOperation::UpdateMedia {
                block_id: RemoteId::new("file-1"),
                local_path: ".loc/media/Roadmap/index.html".into(),
...
fn apply_uploads_local_image_media_before_block_append() {
    let temp = tempdir().expect("tempdir");
    let media_path = temp.path().join(".loc/media/Roadmap/night-sky.jpg");
    fs::create_dir_all(media_path.parent().expect("parent")).expect("mkdir");
    fs::write(&media_path, b"new image bytes").expect("write image");
    let api = Arc::new(RecordingNotionApi::new("2026-06-10T00:00:00.000Z", false));
    let connector = NotionConnector::with_api(NotionConfig::default(), api.clone());
...
fn apply_uploads_local_video_media_before_block_append() {
    let temp = tempdir().expect("tempdir");
    let media_path = temp.path().join(".loc/media/Roadmap/cars.mp4");
    fs::create_dir_all(media_path.parent().expect("parent")).expect("mkdir");
...
fn apply_uploads_common_local_file_media_before_block_append() {
    ...
    let audio_path = temp.path().join(".loc/media/Roadmap/theme.mp3");
    let html_path = temp.path().join(".loc/media/Roadmap/index.html");
    let slides_path = temp.path().join(".loc/media/Roadmap/slides.pptx");
...
fn apply_rejects_local_media_uploads_over_single_file_cap() {
    let temp = tempdir().expect("tempdir");
    let media_path = temp.path().join(".loc/media/Roadmap/large.pdf");
    fs::create_dir_all(media_path.parent().expect("parent")).expect("mkdir");
    ...
    assert!(
        matches!(error, LocalityError::Unsupported(message) if message.contains("larger than 20MB")),
        "{error:?}"
    );
    assert!(
    ...
    let plan = PushPlan::new(
        ...
        vec![PushOperation::UpdateProperties {
            ...
            properties: [
                ...
                (
                    "URL".to_string(),
                    PropertyValue::String("https://example.com/loc".to_string()),
                ),
                (
                    "Files".to_string(),
