...
pub const MACOS_LAUNCHD_LABEL: &str = "ai.codeflash.afs.afsd";
pub const DAEMON_SOCKET_FILENAME: &str = "afsd.sock";
pub const DAEMON_PID_FILENAME: &str = "afsd.pid";
...
fn start_session(
    ...
    let mut command = Command::new(config.afsd_bin);
    command
    ...
    Ok(DaemonProcessStartReport {
        ...
        afsd_bin: config.afsd_bin.to_path_buf(),
    })
}
...
fn launch_agent_plist(config: &DaemonProcessStartConfig<'_>) -> Result<String, DaemonProcessError> {
    ...
    if let Some(tcp_addr) = config.tcp_addr {
        env_vars.push(("AFS_DAEMON_TCP_ADDR".to_string(), tcp_addr.to_string()));
    }
...
"#,
        ...
        afsd_bin = xml_escape(&config.afsd_bin.display().to_string()),
        env_xml = env_xml,
...
mod tests {
    ...
    fn process_paths_use_stable_daemon_filenames() {
        let root = PathBuf::from("/tmp/afs-state");
        let paths = DaemonProcessPaths::for_target(root.clone(), "linux", None);

        assert_eq!(paths.state_root, root);
        assert_eq!(paths.socket, PathBuf::from("/tmp/afs-state/afsd.sock"));
        assert_eq!(paths.pid_file, PathBuf::from("/tmp/afs-state/afsd.pid"));
        assert_eq!(
            paths.metadata_file,
            PathBuf::from("/tmp/afs-state/afsd.manager.json")
        );
        assert_eq!(
            paths.stdout_log,
            PathBuf::from("/tmp/afs-state/logs/afsd.out.log")
        );
        assert_eq!(
            paths.stderr_log,
            PathBuf::from("/tmp/afs-state/logs/afsd.err.log")
        );
    ...
    fn macos_process_paths_include_launch_agent() {
        ...
        assert_eq!(
            paths.launch_agent,
            Some(PathBuf::from(
                "/Users/ada/Library/LaunchAgents/ai.codeflash.afs.afsd.plist"
            ))
        );
    }
    ...
    fn daemon_socket_path_uses_state_root() {
        assert_eq!(
            daemon_socket_path(&PathBuf::from("/tmp/afs-state")),
            PathBuf::from("/tmp/afs-state/afsd.sock")
        );
    }
    ...
    fn launch_agent_plist_contains_daemon_environment() {
        ...
        let config = DaemonProcessStartConfig {
            ...
            paths: &paths,
            afsd_bin: &PathBuf::from("/tmp/afsd"),
        ...
        assert!(plist.contains("<key>AFS_DAEMON_TCP_ADDR</key>"));
