use std::sync::Arc;

use locality_connector::{ChildContainer, Connector, FetchRequest, ListChildrenRequest};
use locality_core::model::{MountId, RemoteId};
use locality_core::{LocalityError, LocalityResult};
use locality_gitlab::{
    GitLabApi, GitLabConfig, GitLabConnector, GitLabContent, GitLabIssue, GitLabMergeRequest,
    GitLabRepository, GitLabRepositoryOwner, GitLabUser,
};

#[test]
fn gitlab_connector_projects_repository_context_as_files() {
    let connector = test_connector();
    let mount_id = MountId::new("gitlab-main");

    let root = connector
        .list_children(ListChildrenRequest {
            mount_id: mount_id.clone(),
            container: ChildContainer::Root,
            parent_path: "".into(),
        })
        .expect("root children");
    assert_eq!(
        root.entries[0].path,
        std::path::PathBuf::from("Repositories")
    );

    let repos = connector
        .list_children(ListChildrenRequest {
            mount_id: mount_id.clone(),
            container: ChildContainer::DirectoryChildren(RemoteId::new("gitlab:repositories")),
            parent_path: "Repositories".into(),
        })
        .expect("repo children");
    assert_eq!(
        repos.entries[0].path,
        std::path::PathBuf::from("Repositories/codeflash-ai/locality")
    );

    let repo_children = connector
        .list_children(ListChildrenRequest {
            mount_id,
            container: ChildContainer::DirectoryChildren(RemoteId::new(
                "gitlab:repo:codeflash-ai/locality",
            )),
            parent_path: "Repositories/codeflash-ai/locality".into(),
        })
        .expect("repo metadata children");
    let paths = repo_children
        .entries
        .iter()
        .map(|entry| entry.path.to_string_lossy().to_string())
        .collect::<Vec<_>>();
    assert_eq!(
        paths,
        vec![
            "Repositories/codeflash-ai/locality/repository.md",
            "Repositories/codeflash-ai/locality/Issues",
            "Repositories/codeflash-ai/locality/Merge Requests",
            "Repositories/codeflash-ai/locality/README.md",
        ]
    );
}

#[test]
fn gitlab_connector_hydrates_issue_markdown() {
    let connector = test_connector();
    let native = connector
        .fetch(FetchRequest {
            remote_id: RemoteId::new("gitlab:issue:codeflash-ai/locality:42"),
        })
        .expect("issue native");
    let document = connector.render(&native).expect("issue render");

    assert!(document.frontmatter.contains("connector: gitlab"));
    assert!(document.frontmatter.contains("kind: issue"));
    assert!(document.frontmatter.contains("number: 42"));
    assert_eq!(document.body, "Fixes the sync bug.\n");
}

#[test]
fn gitlab_connector_is_read_only() {
    let connector = test_connector();

    assert!(connector.supported_push_operations().is_empty());
    assert!(matches!(
        connector.parse(&locality_core::model::CanonicalDocument::new("", "")),
        Err(LocalityError::Unsupported(message)) if message == "GitLab writes"
    ));
}

fn test_connector() -> GitLabConnector {
    GitLabConnector::with_api(
        GitLabConfig::new("test-token"),
        Arc::new(FakeGitLabApi::default()),
    )
}

#[derive(Clone, Debug)]
struct FakeGitLabApi {
    repo: GitLabRepository,
    issue: GitLabIssue,
    merge_request: GitLabMergeRequest,
    readme: GitLabContent,
}

impl Default for FakeGitLabApi {
    fn default() -> Self {
        let repo = GitLabRepository {
            id: 1,
            name: "locality".to_string(),
            path: "locality".to_string(),
            full_name: "codeflash-ai/locality".to_string(),
            owner: GitLabRepositoryOwner {
                name: "CodeFlash AI".to_string(),
                path: "codeflash-ai".to_string(),
                full_path: "codeflash-ai".to_string(),
            },
            description: Some("Filesystem for agents".to_string()),
            html_url: "https://gitlab.com/codeflash-ai/locality".to_string(),
            visibility: "public".to_string(),
            archived: false,
            default_branch: "main".to_string(),
            created_at: "2026-01-01T00:00:00Z".to_string(),
            updated_at: "2026-07-23T00:00:00Z".to_string(),
            updated_at_fallback: None,
        };
        let user = GitLabUser {
            username: "saga4".to_string(),
            web_url: Some("https://gitlab.com/saga4".to_string()),
        };
        Self {
            repo: repo.clone(),
            issue: GitLabIssue {
                id: 42,
                number: 42,
                title: "Fix sync".to_string(),
                body: Some("Fixes the sync bug.".to_string()),
                state: "opened".to_string(),
                html_url: "https://gitlab.com/codeflash-ai/locality/-/issues/42".to_string(),
                created_at: "2026-07-22T00:00:00Z".to_string(),
                updated_at: "2026-07-23T00:00:00Z".to_string(),
                closed_at: None,
                user: Some(user.clone()),
                labels: vec!["sync".to_string()],
            },
            merge_request: GitLabMergeRequest {
                id: 7,
                number: 7,
                title: "Improve connector".to_string(),
                body: Some("Adds connector coverage.".to_string()),
                state: "opened".to_string(),
                html_url: "https://gitlab.com/codeflash-ai/locality/-/merge_requests/7".to_string(),
                created_at: "2026-07-22T00:00:00Z".to_string(),
                updated_at: "2026-07-23T00:00:00Z".to_string(),
                closed_at: None,
                merged_at: None,
                draft: false,
                work_in_progress: false,
                source_branch: Some("connector".to_string()),
                target_branch: Some("main".to_string()),
                user: Some(user),
            },
            readme: GitLabContent {
                name: "README.md".to_string(),
                path: "README.md".to_string(),
                sha: "readme-sha".to_string(),
                content: "IyBMb2NhbGl0eQo=".to_string(),
                encoding: "base64".to_string(),
            },
        }
    }
}

impl GitLabApi for FakeGitLabApi {
    fn current_user(&self) -> LocalityResult<GitLabUser> {
        Ok(GitLabUser {
            username: "saga4".to_string(),
            web_url: None,
        })
    }

    fn list_repositories(&self) -> LocalityResult<Vec<GitLabRepository>> {
        Ok(vec![self.repo.clone()])
    }

    fn get_repository(&self, full_name: &str) -> LocalityResult<GitLabRepository> {
        if full_name == "codeflash-ai/locality" {
            return Ok(self.repo.clone());
        }
        Err(LocalityError::RemoteNotFound(full_name.to_string()))
    }

    fn get_readme(
        &self,
        full_name: &str,
        default_branch: &str,
    ) -> LocalityResult<Option<GitLabContent>> {
        if full_name == "codeflash-ai/locality" && default_branch == "main" {
            return Ok(Some(self.readme.clone()));
        }
        Ok(None)
    }

    fn list_issues(&self, _full_name: &str) -> LocalityResult<Vec<GitLabIssue>> {
        Ok(vec![self.issue.clone()])
    }

    fn get_issue(&self, _full_name: &str, number: u64) -> LocalityResult<GitLabIssue> {
        if number == self.issue.number {
            return Ok(self.issue.clone());
        }
        Err(LocalityError::RemoteNotFound(number.to_string()))
    }

    fn list_merge_requests(&self, _full_name: &str) -> LocalityResult<Vec<GitLabMergeRequest>> {
        Ok(vec![self.merge_request.clone()])
    }

    fn get_merge_request(
        &self,
        _full_name: &str,
        number: u64,
    ) -> LocalityResult<GitLabMergeRequest> {
        if number == self.merge_request.number {
            return Ok(self.merge_request.clone());
        }
        Err(LocalityError::RemoteNotFound(number.to_string()))
    }
}
