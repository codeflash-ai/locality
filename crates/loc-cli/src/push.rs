...
use localityd::push::{PushJobAction, execute_push_job, execute_push_job_with_content_root};
use serde::Serialize;
        ...
        + MountLiveModeRepository,
{
    ...
    let status = match run_status(
        ...
        Ok(status) => status,
        Err(StatusError::Store(locality_store::StoreError::EntityPathMissing { .. }))
...
fn push_error_suggested_fix(
    ...
    if error.code == "guardrail" && error.message.contains("changed since last sync") {
        if let Some(parent_path) = create_entity_parent_pull_path(path, plan) {
            ...
            return Some(format!(
                "run `loc pull {parent_path}` to update the parent from remote, then rerun `loc push {path} -y`"
            ));
        }

        let path = shell_quote(path);
        return Some(format!(
            "run `loc pull {path}` to update from remote, resolve any conflicts, then rerun `loc push {path} -y`"
        ));
    }
    None
}
...
fn push_stage_name(stage: &locality_core::push::PushStage) -> &'static str {
    match stage {
        locality_core::push::PushStage::ParseAndValidate => "parse_and_validate",
        locality_core::push::PushStage::Diff => "diff",
        locality_core::push::PushStage::PlanAndConfirm => "plan_and_confirm",
        locality_core::push::PushStage::ConcurrencyCheckAndApply => "concurrency_check_and_apply",
        locality_core::push::PushStage::JournalAndReconcile => "journal_and_reconcile",
    }
}
