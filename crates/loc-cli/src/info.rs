...
use locality_core::path_projection::{page_container_path, page_listing_parent_path};
use locality_store::{
    EntityRecord, EntityRepository, JournalRepository, MountRepository, StoreError,
};
use localityd::{file_provider, source::source_display_name};
use serde::Serialize;
    ...
    S: MountRepository + EntityRepository + JournalRepository,
{
    ...
    Ok(InfoReport {
        ...
        subject: InfoSubject {
            ...
            path: locality_platform::logical_path_display(&relative_path),
...
fn info_entity(access_root: &Path, entity: &EntityRecord) -> InfoEntity {
    InfoEntity {
        ...
        absolute_path: locality_platform::join_logical_path(access_root, &entity.path)
            .display()
            .to_string(),
...
fn suggestions(subject_context: &SubjectContext) -> Vec<String> {
    match subject_context.role {
        InfoRole::MountRoot | InfoRole::PageWorkspace | InfoRole::DatabaseDirectory => vec![
            "loc status .".to_string(),
            "loc pull <path-to-stub>".to_string(),
            "loc diff <path-before-push>".to_string(),
        ],
        InfoRole::PageFile => vec![
            "loc status <path>".to_string(),
            "loc pull <path>".to_string(),
            "loc diff <path>".to_string(),
        ],
        InfoRole::UntrackedPath => vec!["loc status .".to_string()],
        InfoRole::ProjectedDirectory | InfoRole::Asset | InfoRole::UnknownEntity => {
            vec!["loc status <path>".to_string()]
        }
    }
}
