...
#[test]
fn existing_page_title_change_reconciles_without_duplicate_projection() {
    let fixture = E2eFixture::new();
    let mut store = InMemoryStateStore::new();
    let api = Arc::new(MutableNotionApi::new());
    let connector = NotionConnector::with_api(NotionConfig::default(), api.clone());
    ...
    let original_path = fixture.page_file();
    let original = fs::read_to_string(&original_path).expect("read pulled page");
    assert!(original.contains("title: \"Initial Idea\""), "{original}");
    fs::write(
        &original_path,
        original.replace("title: \"Initial Idea\"", "title: \"Renamed Idea\""),
    )
    .expect("write title edit");

    let diff = run_diff(&store, &original_path).expect("diff title edit");
    assert_eq!(diff.action, "confirm_plan", "{diff:#?}");
    let plan = diff.plan.as_ref().expect("title edit plan");
    assert_eq!(plan.summary.properties_updated, 1, "{plan:#?}");
    assert_eq!(plan.summary.entities_created, 0, "{plan:#?}");
    ...
    .expect("push title edit");
    assert!(push.ok, "{push:#?}");
    assert_eq!(push.action, "reconciled", "{push:#?}");
    assert_eq!(push.changed_remote_ids, vec!["page-1"], "{push:#?}");

    run_pull(&mut store, &connector, &fixture.root).expect("pull after title edit");

    let projected_pages = collect_files(&fixture.root)
        .into_iter()
    ...
    assert_eq!(
        projected_pages.len(),
        1,
        "title-only change should leave exactly one projected page.md: {projected_pages:#?}"
    );

    let renamed_path = fixture.root.join("renamed-idea/page.md");
    assert!(
        renamed_path.exists(),
        "renamed projection should exist after pull: {projected_pages:#?}"
    );
    assert!(
        !fixture.root.join("initial-idea/page.md").exists(),
        "old projection should not remain as a duplicate"
    );
    let entity = store
        .get_entity(&fixture.mount_id, &RemoteId::new("page-1"))
        .expect("get renamed page entity")
        .expect("renamed page entity");
    assert_eq!(entity.path, PathBuf::from("renamed-idea/page.md"));
    assert_eq!(entity.title, "Renamed Idea");
}

#[test]
fn hydrated_page_remote_title_change_pull_reconciles_without_duplicate_projection() {
    let fixture = E2eFixture::new();
    let mut store = InMemoryStateStore::new();
    let api = Arc::new(MutableNotionApi::new());
    let connector = NotionConnector::with_api(NotionConfig::default(), api.clone());
    ...
    assert_eq!(
        ...
        Path::new("initial-idea/page.md")
    );
    let original = fs::read_to_string(&original_path).expect("read hydrated page");
    assert!(original.contains("title: \"Initial Idea\""), "{original}");
    assert!(original.contains("First paragraph."), "{original}");

    replace_mutable_page_title_and_version(&api, "Remote Renamed Idea", "2026-06-10T00:00:02.000Z");

    let pull = run_pull(&mut store, &connector, &fixture.root).expect("pull remote title change");
    assert!(pull.ok, "{pull:#?}");
    ...
    assert_eq!(
        projected_pages.len(),
        1,
        "remote title change should leave exactly one projected page.md: {projected_pages:#?}"
    );

    let renamed_path = fixture.root.join("remote-renamed-idea/page.md");
    assert!(
        renamed_path.exists(),
        "renamed remote projection should exist after pull: {projected_pages:#?}"
    );
    assert!(
        !fixture.root.join("initial-idea/page.md").exists(),
        "old hydrated projection should not remain as a duplicate"
    );
    let renamed = fs::read_to_string(&renamed_path).expect("read renamed hydrated page");
    assert!(
        renamed.contains("title: \"Remote Renamed Idea\""),
        "{renamed}"
    );
    assert!(renamed.contains("First paragraph."), "{renamed}");

    let entity = store
        .get_entity(&fixture.mount_id, &RemoteId::new("page-1"))
        .expect("get remote-renamed page entity")
        .expect("remote-renamed page entity");
    assert_eq!(entity.path, PathBuf::from("remote-renamed-idea/page.md"));
    assert_eq!(entity.title, "Remote Renamed Idea");
...
fn live_existing_page_title_change_reconciles_without_duplicate_projection() {
    let env = LiveEnv::from_env();
    let api = HttpNotionApi::new(live_notion_config());
    let mut cleanup = LiveCleanup::new(api);
    let original_title = format!("Locality live title edit {}", unique_suffix());
    let renamed_title = format!("Locality live title edited {}", unique_suffix());
    let body = "Live local title edit body should stay with one page projection.";
    let scratch = cleanup.create_page(
        &env.parent_page_id,
        &original_title,
        vec![paragraph_child(body)],
    );
    ...
    assert!(
        original.contains(&format!("title: \"{original_title}\"")),
        "{original}"
    );
    assert!(original.contains(body), "{original}");

    fs::write(
        &page_path,
        original.replace(
            &format!("title: \"{original_title}\""),
            &format!("title: \"{renamed_title}\""),
        ),
    )
    .expect("write live title edit");

    let diff = run_diff(&store, &page_path).expect("diff live title edit");
    assert_eq!(diff.action, "confirm_plan", "{diff:#?}");
    let plan = diff.plan.as_ref().expect("live title edit plan");
    assert_eq!(plan.summary.properties_updated, 1, "{plan:#?}");
    assert_eq!(plan.summary.entities_created, 0, "{plan:#?}");
    ...
    .expect("push live title edit");
    assert!(push.ok, "{push:#?}");
    assert_eq!(push.action, "reconciled", "{push:#?}");
    assert_eq!(
        push.changed_remote_ids,
        vec![scratch.id.clone()],
        "{push:#?}"
    );
    wait_for_live_page_title(
        ...
        &scratch.id,
        &renamed_title,
        "live local title edit",
    );

    run_pull(&mut store, &connector, &fixture.root).expect("pull live title edit");
    let projected_pages = collect_files(&fixture.root)
        .into_iter()
        .filter(|path| file_name(path) == "page.md")
        .collect::<Vec<_>>();
    assert_eq!(
        projected_pages.len(),
        1,
        "live title edit should leave exactly one projected page.md: {projected_pages:#?}"
    );

    let renamed_path = fixture
        .root
        .join(slug_for_test(&renamed_title))
        .join("page.md");
    assert!(
        renamed_path.exists(),
        "renamed live projection should exist after pull: {projected_pages:#?}"
    );
    assert!(
        !fixture
            .root
            .join(slug_for_test(&original_title))
            .join("page.md")
            .exists(),
        "old live projection should not remain as a duplicate"
    );
    let renamed = fs::read_to_string(&renamed_path).expect("read renamed live page");
    assert!(
        renamed.contains(&format!("title: \"{renamed_title}\"")),
        "{renamed}"
    );
    assert!(renamed.contains(body), "{renamed}");
}

#[test]
#[ignore = "requires Notion credentials (NOTION_TOKEN or ~/.loc credentials) and LOCALITY_NOTION_LIVE_PARENT_PAGE; creates and archives scratch Notion content"]
fn live_hydrated_page_remote_title_change_pull_reconciles_without_duplicate_projection() {
    let env = LiveEnv::from_env();
    let api = HttpNotionApi::new(live_notion_config());
    let mut cleanup = LiveCleanup::new(api);
    let original_title = format!("Locality live remote title {}", unique_suffix());
    let renamed_title = format!("Locality live remote renamed {}", unique_suffix());
    let body = "Live remote title edit body should stay hydrated once.";
    let scratch = cleanup.create_page(
        &env.parent_page_id,
        &original_title,
        vec![paragraph_child(body)],
    ...
    cleanup
        ...
        .update_page(
            ...
            json!({
                "properties": {
                    "title": {
                        "title": rich_text_json(&renamed_title)
                    }
                }
            }),
        )
        .expect("rename live page remotely");
    wait_for_live_page_title(
        &cleanup.api,
        &scratch.id,
        &renamed_title,
        "live remote title edit",
    );

    let pull =
        run_pull(&mut store, &connector, &fixture.root).expect("pull live remote title edit");
    assert!(pull.ok, "{pull:#?}");
    ...
    assert_eq!(
        projected_pages.len(),
        1,
        "live remote title edit should leave exactly one projected page.md: {projected_pages:#?}"
    );
    ...
    assert!(
        renamed_path.exists(),
        "renamed live remote projection should exist after pull: {projected_pages:#?}"
    );
    assert!(
        !page_path.exists(),
        "old hydrated live projection should not remain as a duplicate"
    );
    let renamed = fs::read_to_string(&renamed_path).expect("read live remote renamed page");
    assert!(
        renamed.contains(&format!("title: \"{renamed_title}\"")),
        "{renamed}"
    );
    ...
    let entity = store
        ...
        .expect("get live remote-renamed entity")
        .expect("live remote-renamed entity");
    assert_eq!(
        entity.path,
        PathBuf::from(slug_for_test(&renamed_title)).join("page.md")
    );
    assert_eq!(entity.title, renamed_title);
    assert_eq!(entity.hydration, HydrationState::Hydrated);
}
...
fn wait_for_live_page_title(api: &HttpNotionApi, page_id: &str, title: &str, context: &str) {
    let deadline = Instant::now() + Duration::from_secs(12);
    let mut last_title = String::new();

    while Instant::now() < deadline {
        match api.retrieve_page(page_id) {
            Ok(page) => {
                last_title = notion_page_title_for_test(&page);
                if last_title == title {
                    return;
                }
            }
            Err(error) => {
                last_title = error.to_string();
            }
        }
        thread::sleep(Duration::from_millis(250));
    }
    ...
    panic!(
        "{context}: live page `{page_id}` did not expose title `{title}` before timeout; last observed title: {last_title}"
    );
}

fn notion_page_title_for_test(page: &PageDto) -> String {
    page.properties
        .values()
        ...
        .map(|property| {
            property
                ...
                .iter()
                .map(|rich_text| {
                    if rich_text.plain_text.is_empty() {
                        rich_text
                            .text
                            .as_ref()
