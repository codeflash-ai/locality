...
const LIVE_WORKSPACE_ROOT_ENV: &str = "LOCALITY_LIVE_WORKSPACE_ROOT";
const LIVE_WORKSPACE_FILE_ENV: &str = "LOCALITY_LIVE_WORKSPACE_FILE";
...
fn live_workspace_pull_edit_pull_push_regression() {
    ...
    json_ok(command(loc, ["pull"]).arg(&root).arg("--json"));
    json_ok(command(loc, ["pull"]).arg(&file).arg("--json"));
    assert_stable_metadata(&file);

    let _restore = RestoreOnDrop::new(loc, file.clone());
    let original = fs::read_to_string(&file).expect("read mounted file");
    let marker = format!("Locality live workspace regression {}", unique_suffix());
    fs::write(
        &file,
    ...
    let pull_dirty = json_allow_failure(command(loc, ["pull"]).arg(&file).arg("--json"));
    ...
    let pushed = json_ok(command(loc, ["push"]).arg(&file).arg("-y").arg("--json"));
    assert_eq!(pushed["ok"], true, "{pushed:#?}");

    let pulled_after_push = json_ok(command(loc, ["pull"]).arg(&file).arg("--json"));
...
fn command<const N: usize>(loc: &str, args: [&str; N]) -> Command {
...
fn json_allow_failure(command: &mut Command) -> Value {
    parse_json(command.output().expect("run loc command"))
}
...
impl<'a> RestoreOnDrop<'a> {
    fn new(loc: &'a str, file: PathBuf) -> Self {
        Self { loc, file }
    }
}
