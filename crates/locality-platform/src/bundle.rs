...
mod tests {
    ...
    fn windows_executables_use_exe_suffix() {
        assert_eq!(executable_filename_for_target("loc", "windows"), "loc.exe");
        assert_eq!(
            executable_filename_for_target("loc.exe", "windows"),
            "loc.exe"
        );
    }
    ...
    fn unix_executables_preserve_name() {
        assert_eq!(executable_filename_for_target("loc", "macos"), "loc");
        assert_eq!(
            executable_filename_for_target("localityd", "linux"),
            "localityd"
        );
    }
    ...
    fn sidecar_candidates_include_plain_and_tauri_triple_names() {
        ...
        assert_eq!(
            ...
            vec![
                bundle_dir.join("localityd.exe"),
                bundle_dir.join("localityd-x86_64-pc-windows-msvc.exe"),
            ]
        );
    }
}
