...
//! AFS should keep across connectors: browse metadata lazily, materialize on
...
impl BehaviorFixture {
    fn new() -> Self {
        let root = unique_temp_path("afs-architecture-behavior-root");
        let state_root = unique_temp_path("afs-architecture-behavior-state");
...
fn assert_child(children: &[afsd::virtual_fs::VirtualFsItem], filename: &str, kind: EntityKind) {
    let child = children
        .iter()
...
fn assert_folder(children: &[afsd::virtual_fs::VirtualFsItem], filename: &str) {
    let child = children
        .iter()
        .find(|child| child.filename == filename)
...
fn entry_issue_codes(report: &afs_cli::status::StatusReport, path: &str) -> Vec<String> {
    report
        .mounts
        .iter()
        .flat_map(|mount| &mount.entries)
