...
impl ProjectionFixture {
    fn new(projection: ProjectionMode) -> Self {
        let root = temp_path("afs-projection-contract-root");
        let state_root = temp_path("afs-projection-contract-state");
...
impl Connector for ContractSource {
    ...
    fn enumerate(&self, _request: EnumerateRequest) -> AfsResult<Vec<TreeEntry>> {
        Err(AfsError::NotImplemented("contract enumerate"))
    }
    ...
    fn fetch(&self, _request: FetchRequest) -> AfsResult<NativeEntity> {
        Err(AfsError::NotImplemented("contract fetch"))
    }

    fn render(&self, _entity: &NativeEntity) -> AfsResult<CanonicalDocument> {
        Err(AfsError::NotImplemented("contract render"))
    }

    fn parse(&self, _document: &CanonicalDocument) -> AfsResult<ParsedEntity> {
        Err(AfsError::NotImplemented("contract parse"))
    }
    ...
    fn apply_undo(&self, _request: ApplyUndoRequest<'_>) -> AfsResult<ApplyUndoResult> {
        Err(AfsError::NotImplemented("contract undo"))
    }
}

fn assert_child_folder(children: &[afsd::virtual_fs::VirtualFsItem], filename: &str) {
    let child = children
        .iter()
...
fn assert_child_file(children: &[afsd::virtual_fs::VirtualFsItem], filename: &str) {
    let child = children
        .iter()
        .find(|child| child.filename == filename)
