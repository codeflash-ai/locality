...
fn local_input<'a>(
    entity: &EntityRecord,
    relative_path: &Path,
    parsed: Result<&'a afs_core::canonical::ParsedCanonicalDocument, &'a CanonicalParseError>,
) -> RemoteChangeInput<'a> {
    match parsed {
        Ok(parsed) => {
            match parsed.remote_id() {
                ...
                Some(_) => {
                    return RemoteChangeInput::Unavailable(RemoteChangeIssue::new(
                        ...
                        format!(
                            "frontmatter `afs.id` does not match the entity mapped to `{}`",
                ...
                None => {
                    return RemoteChangeInput::Unavailable(RemoteChangeIssue::new(
                        ...
                        format!(
                            "frontmatter `afs.id` is missing for the entity mapped to `{}`",
                            relative_path.display()
                        ),
                    ));
                }
...
fn remote_body_start_line(shadow: &afs_core::shadow::ShadowDocument) -> usize {
    shadow
        .blocks
        .iter()
