...
use afs_platform::capabilities::projection_cli_value;
use afs_platform::mount_cli_capabilities;
use afs_store::{
...
pub fn run_doctor(options: DoctorOptions) -> DoctorReport {
    let state_root = options
        .state_root
        .unwrap_or_else(afs_platform::default_state_root);
    ...
    if state_store.exists && state_store.readable {
        match SqliteStateStore::open(state_root.clone()) {
            ...
            Err(error) => findings.push(DoctorFinding::global(
                ...
                format!("Could not open AFS state: {error}"),
                None,
            )),
        }
    }
...
pub fn print_doctor_report(report: &DoctorReport) {
    println!("AFS doctor: {}", report.status.as_str());
    ...
    match run_daemon_control(&args) {
        ...
        Err(error) => {
            findings.push(DoctorFinding::global(
                ...
                Some("afs daemon status".to_string()),
            ));
            None
        }
    }
...
fn state_store_status(state_root: &Path, findings: &mut Vec<DoctorFinding>) -> DoctorStateStore {
    let compatibility = match SqliteStateStore::inspect_compatibility(state_root.to_path_buf()) {
        ...
        Err(error) => {
            ...
            findings.push(DoctorFinding::global(
                ...
                format!("Could not inspect AFS state store: {error}"),
                None,
            ));
    ...
    let readable = matches!(
        ...
        afs_store::StateCompatibilityStatus::Ready
            | afs_store::StateCompatibilityStatus::Migratable
    );
    ...
    if !compatibility.db_exists {
        findings.push(DoctorFinding::global(
            DoctorSeverity::Info,
            "state_store_missing",
            "No AFS state store exists yet; connect and mount a workspace to initialize AFS.",
            Some("afs connect notion".to_string()),
        ));
    } else if !readable {
        findings.push(DoctorFinding::global(
            DoctorSeverity::Error,
            ...
            format!(
                "AFS state store is not readable by this binary: {:?}",
...
fn connection_credential_status(
    credentials: &dyn afs_store::CredentialStore,
    connection: &ConnectionRecord,
    findings: &mut Vec<DoctorFinding>,
) -> DoctorCheckState {
    ...
    match credentials.get(&connection.secret_ref) {
        ...
        Err(afs_store::CredentialError::NotFound(_)) => {
            findings.push(DoctorFinding::connection(
                DoctorSeverity::Error,
                "connection_credential_missing",
                format!(
        ...
        .map(|mount| {
                ...
                && daemon.is_none_or(|daemon| daemon.state != DaemonRunState::Running.as_str())
            {
                findings.push(DoctorFinding::mount(
                    ...
                    format!("Virtual mount `{}` needs afsd running.", mount.mount_id.0),
                    &mount.mount_id.0,
                    Some("afs daemon start".to_string()),
                ));
            }
...
fn macos_provider_status(mount: &MountConfig, findings: &mut Vec<DoctorFinding>) -> DoctorProvider {
    #[cfg(target_os = "macos")]
    {
        ...
        add_provider_findings(mount, &provider, findings, "afs file-provider register");
        provider
    }
...
fn linux_provider_status(mount: &MountConfig, findings: &mut Vec<DoctorFinding>) -> DoctorProvider {
    #[cfg(target_os = "linux")]
    {
        let helper = file_provider::afs_fuse_helper_path();
...
fn windows_provider_status(
    ...
    add_provider_findings(mount, &provider, findings, "afs file-provider start");
    provider
}
