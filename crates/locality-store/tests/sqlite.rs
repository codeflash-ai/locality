...
fn sqlite_store_seeds_state_compatibility_components() {
    ...
    assert_eq!(
        ...
        vec![
            ...
            ("core:schema".to_string(), "schema".to_string(), 24, 1, 1, 0),
            (
...
#[test]
fn sqlite_schema_snapshot_matches_v24_contract() {
    let fixture = SqliteFixture::new();
    let store = fixture.open();
    let connection = Connection::open(&store.db_path).expect("raw connection");
...
#[test]
fn sqlite_store_migrates_v18_to_v24_without_discarding_pending_work() {
    let fixture = SqliteFixture::new();
    let mut store = fixture.open();
    store
    ...
    let migration_count: i64 = connection
        .query_row(
            "SELECT COUNT(*) FROM state_migrations
             WHERE migration_id = 'schema-18-to-24'",
            [],
            |row| row.get(0),
        )
...
fn sqlite_store_reports_v12_state_as_migratable_then_migrates() {
    ...
    let migration_count: i64 = connection
        .query_row(
            "SELECT COUNT(*) FROM state_migrations WHERE migration_id = 'schema-12-to-24'",
            [],
            |row| row.get(0),
        )
    ...
    let after =
        SqliteStateStore::inspect_compatibility(fixture.state_root.clone()).expect("inspect v24");
    assert_eq!(after.status, StateCompatibilityStatus::Ready);
}
...
fn sqlite_store_migrates_journals_component_v2_to_v3_without_rewriting_rows() {
    ...
    assert_eq!(before_user_version, 24);
    assert_eq!(after_user_version, before_user_version);
...
#[test]
fn sqlite_store_v14_missing_live_mode_component_migrates_to_v24() {
    let fixture = SqliteFixture::new();
    let mount_point_root = fixture
        .state_root
        .parent()
    ...
    let migration_count: i64 = connection
        .query_row(
            "SELECT COUNT(*)
             ...
             WHERE migration_id = 'schema-16-to-24'",
            [],
            |row| row.get(0),
        )
...
fn sqlite_store_migrates_v17_mounts_with_default_settings_json() {
    ...
    let migration_count: i64 = connection
        .query_row(
            "SELECT COUNT(*)
             FROM state_migrations
             WHERE migration_id = 'schema-17-to-24'",
            [],
            |row| row.get(0),
        )
        .expect("schema migration count");
