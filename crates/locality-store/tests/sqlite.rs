...
use locality_store::{
    ...
    VirtualMutationRecord, VirtualMutationRepository, WorkspaceBindingRepository,
};
...
fn sqlite_store_seeds_state_compatibility_components() {
    ...
    assert_eq!(
        ...
        vec![
            ...
            ("core:schema".to_string(), "schema".to_string(), 29, 1, 1, 0),
            (
            ...
            (
                "durable:hosted_workspaces".to_string(),
                "durable_transaction".to_string(),
                2,
                2,
                1,
...
#[test]
fn sqlite_schema_snapshot_matches_v29_contract() {
    let fixture = SqliteFixture::new();
    let store = fixture.open();
    let connection = Connection::open(&store.db_path).expect("raw connection");
...
hosted_workspace_attachments: api_origin, profile_id, credential_ref, root, profile_revision, layout_version, layout_digest, updated_at
hosted_workspace_mount_mappings: api_origin, profile_id, portable_mount_id, local_mount_id, mount_target, target_collision_key, active, first_seen_revision, last_seen_revision
hosted_workspace_pending_cleanups: cleanup_id, api_origin, profile_id, credential_ref, root, profile_revision, layout_version, layout_digest, created_at
hosted_workspace_pending_mounts: transition_id, portable_mount_id, local_mount_id, mount_target, target_collision_key, first_seen_revision
hosted_workspace_pending_transitions: transition_id, api_origin, profile_id, credential_ref, target_root, transition_kind, profile_revision, layout_version, layout_digest, base_profile_revision, base_layout_digest, base_root, created_at
...
#[test]
fn sqlite_store_migrates_v18_to_v29_without_discarding_pending_work() {
    let fixture = SqliteFixture::new();
    let mut store = fixture.open();
    store
    ...
    let migration_count: i64 = connection
        .query_row(
            "SELECT COUNT(*) FROM state_migrations
             WHERE migration_id = 'schema-18-to-29'",
            [],
            |row| row.get(0),
        )
...
#[test]
fn sqlite_store_migrates_released_workspace_v21_with_generation_tables_absent() {
    let fixture = SqliteFixture::new();
    let mut store = fixture.open();
    store
        .save_mount(fixture.mount_config())
        .expect("save mount");
    let pending_journal = journal_entry("push-workspace-v21", JournalStatus::Prepared);
    store
        .append_journal(pending_journal.clone())
        .expect("save pending journal");
    let db_path = store.db_path.clone();
    ...
    let connection = Connection::open(&db_path).expect("raw workspace v21 connection");
    connection
        .execute_batch(
            "DROP TABLE generation_inode_evidence;
             ...
             UPDATE state_components SET version = 21
             WHERE component_id = 'core:schema';
             PRAGMA user_version = 21;",
        )
        .expect("downgrade fixture to released workspace v21 shape");
    drop(connection);
    ...
    assert_eq!(
        connection
            ...
            .expect("generation mount column"),
        1
    );
}

#[test]
fn sqlite_store_migrates_freshness_v24_with_workspace_binding_absent() {
    let fixture = SqliteFixture::new();
    let mut store = fixture.open();
    store
        .save_mount(fixture.mount_config())
        .expect("save mount");
    let pending_journal = journal_entry("push-freshness-v24", JournalStatus::Prepared);
    store
        .append_journal(pending_journal.clone())
        .expect("save pending journal");
    fs::write(
        fixture.mount_root.join("pending.md"),
        b"pending local bytes",
    )
    .expect("write pending local bytes");
    let db_path = store.db_path.clone();
    drop(store);

    let connection = Connection::open(&db_path).expect("raw freshness v24 connection");
    connection
        .execute_batch(
        ...
        .expect("downgrade fixture to freshness v24 shape");
    drop(connection);
    ...
    let migration_count: i64 = connection
        .query_row(
            "SELECT COUNT(*) FROM state_migrations
             WHERE migration_id = 'schema-24-to-29'",
            [],
            |row| row.get(0),
        )
        .expect("migration history");
    ...
    assert_eq!(
        fs::read(fixture.mount_root.join("pending.md")).expect("read pending local bytes"),
        b"pending local bytes"
    );
}
...
fn sqlite_store_reports_v12_state_as_migratable_then_migrates() {
    ...
    let migration_count: i64 = connection
        .query_row(
            "SELECT COUNT(*) FROM state_migrations WHERE migration_id = 'schema-12-to-29'",
            [],
            |row| row.get(0),
        )
    ...
    let after =
        SqliteStateStore::inspect_compatibility(fixture.state_root.clone()).expect("inspect v26");
...
fn sqlite_store_migrates_journals_component_v2_to_v3_without_rewriting_rows() {
    ...
    assert_eq!(before_user_version, 29);
...
#[test]
fn sqlite_store_v14_missing_live_mode_component_migrates_to_v29() {
    let fixture = SqliteFixture::new();
    let mount_point_root = fixture
        .state_root
        .parent()
...
fn remounting_same_mount_id_to_different_connection_clears_source_scoped_state() {
    ...
    settle_source_reset_sensitive_state(&mut store, &fixture.mount_id);
    assert_eq!(
    ...
    let migration_count: i64 = connection
        .query_row(
            "SELECT COUNT(*)
             ...
             WHERE migration_id = 'schema-16-to-29'",
            [],
            |row| row.get(0),
        )
...
fn sqlite_store_migrates_v17_mounts_with_default_settings_json() {
    ...
    let migration_count: i64 = connection
        .query_row(
            "SELECT COUNT(*)
             FROM state_migrations
             WHERE migration_id = 'schema-17-to-29'",
            [],
            |row| row.get(0),
        )
        .expect("schema migration count");
...
fn settle_source_reset_sensitive_state(store: &mut SqliteStateStore, mount_id: &MountId) {
    store
        .delete_virtual_mutation(mount_id, "local:draft")
        .expect("settle virtual mutation before source reset");
    store
        .update_journal_status(&PushId("push-1".to_string()), JournalStatus::Reconciled)
        .expect("settle push journal before source reset");
}
