...
use locality_core::model::{CanonicalDocument, EntityKind, RemoteId};
use locality_core::validation::{validate_directive_syntax, validate_frontmatter_identity};
...
fn parses_frontmatter_identity_directives_and_renders_stably() {
    ...
    let loc = parsed.frontmatter.loc.as_ref().expect("loc metadata");
    assert_eq!(loc.id, Some(RemoteId::new("page-1")));
    assert_eq!(loc.entity_type, Some(EntityKind::Page));
    assert_eq!(loc.parent, Some(RemoteId::new("parent-1")));
...
#[test]
fn parses_afs_frontmatter_and_directives_as_loc_compatibility_aliases() {
    let input = r#"---
afs:
  id: page-1
  type: page
...
"#;
    let parsed = parse_canonical_markdown(input).expect("afs-compatible canonical document");

    let loc = parsed
        .frontmatter
        .loc
        .as_ref()
        .expect("normalized loc metadata");
    assert_eq!(loc.id, Some(RemoteId::new("page-1")));
    assert_eq!(loc.parent, Some(RemoteId::new("parent-1")));
    assert_eq!(loc.entity_type, Some(EntityKind::Page));
    assert_eq!(parsed.directives.len(), 1);
    ...
    assert_eq!(
        ...
        r#"::afs{id=block-1 type=synced_block title="Shared header"}"#
    );
}
...
fn directive_syntax_validation_reports_malformed_and_missing_fields() {
    let input = "---\nloc:\n  id: page-1\n  type: page\n  synced_at: now\n  remote_edited_at: now\ntitle: Bad directives\n---\n::loc{id=block-1 type=synced_block\n::loc{type=synced_block}\n::loc{id=block-2}\n";
    let parsed = parse_canonical_markdown(input).expect("parseable document");
