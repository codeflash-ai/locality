...
use locality_core::conflict::BlockChangeSet;
use locality_core::hydration::{HydrationPolicy, should_eager_hydrate};
use locality_core::model::{
    CanonicalBlock, CanonicalDocument, EntityKind, HydrationState, MountId, RemoteId, SourceSpan,
    TreeEntry,
};
use locality_core::planner::{GuardrailDecision, GuardrailPolicy, PushOperation, PushPlan};
use locality_core::push::evaluate_guardrails;
use locality_core::sync::{
    SyncDecision, ThreeTreeSnapshot, classify_changes, classify_colliding_edits,
};
use locality_core::validation::{
...
fn hydration_ladder_allows_expected_transitions() {
    ...
    assert!(HydrationState::Virtual.can_transition_to(&HydrationState::Dirty));
    assert!(HydrationState::Virtual.can_transition_to(&HydrationState::Conflicted));
    assert!(HydrationState::Stub.can_transition_to(&HydrationState::Hydrated));
    assert!(HydrationState::Stub.can_transition_to(&HydrationState::Dirty));
    assert!(HydrationState::Stub.can_transition_to(&HydrationState::Conflicted));
...
fn directive_validation_allows_move_and_delete_but_rejects_mangle_and_invention() {
    ...
    let mangled =
        CanonicalDocument::new("---\n---", "").with_blocks(vec![CanonicalBlock::directive(
            RemoteId::new("block-1"),
            "synced_block",
            "::loc{id=block-1 type=synced_block title=\"edited\"}",
        )]);
    assert_eq!(
    ...
    let invented =
        CanonicalDocument::new("---\n---", "").with_blocks(vec![CanonicalBlock::directive(
            ...
            "synced_block",
            "::loc{id=invented type=synced_block}",
        )]);
    assert_eq!(
