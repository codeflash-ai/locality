...
pub fn rendered_bodies_equivalent(left: &str, right: &str) -> bool {
    left == right || normalize_loc_media_hrefs(left) == normalize_loc_media_hrefs(right)
}

fn normalize_loc_media_hrefs(body: &str) -> String {
    ...
    while let Some(link_start) = rest.find("](") {
        ...
        normalized.push_str(&normalize_loc_media_target(target));
        normalized.push(')');
...
fn normalize_loc_media_target(target: &str) -> String {
    if target.contains("://") {
        return target.to_string();
    }

    if let Some(media_start) = target.find("/files/.loc/media/") {
        let media_path = &target[media_start + "/files/".len()..];
        return media_path.to_string();
    }

    let Some(media_start) = target.find(".loc/media/") else {
        return target.to_string();
    };
...
#[cfg(test)]
mod tests {
    use super::{normalize_loc_media_hrefs, rendered_bodies_equivalent};

    #[test]
    fn rendered_bodies_are_equivalent_when_local_media_href_shape_differs() {
        let shadow = "# Page\n\n![Image](/tmp/loc-content/notion-main/files/.loc/media/getting-started-3-new/image-fb3123d34d04464487428b0f2557e4a0.jpg)\n";
        let remote = "# Page\n\n![Image](../.loc/media/getting-started-3-new/image-fb3123d34d04464487428b0f2557e4a0.jpg)\n";
    ...
    fn rendered_bodies_are_not_equivalent_for_different_media_files() {
        let old = "![Image](../.loc/media/page/image-111.jpg)\n";
        let new = "![Image](../.loc/media/page/image-222.jpg)\n";

        assert!(!rendered_bodies_equivalent(old, new));
    }

    #[test]
    fn normalize_loc_media_hrefs_leaves_external_urls_unchanged() {
        let body = "[Link](https://example.com/.loc/media/page/image-111.jpg)\n";

        assert_eq!(normalize_loc_media_hrefs(body), body);
    }
}
