...
pub struct PushApproval {
    /// Equivalent to `loc push -y`: allow safe non-empty plans to proceed
    /// without an interactive prompt.
    pub assume_yes: bool,
    /// Equivalent to `loc push --confirm`: allow plans that tripped destructive
...
) -> LocalityResult<PushExecutionResult>
where
    H: PushExecutionHost,
{
...
fn mark_failed<J>(journal: &mut J, push_id: &PushId, error: &LocalityError) -> LocalityResult<()>
where
    J: JournalStore,
{
...
fn mark_reverted<J>(journal: &mut J, push_id: &PushId) -> LocalityResult<()>
where
    J: JournalStore,
{
