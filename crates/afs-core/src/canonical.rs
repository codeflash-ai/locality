...
//!
//! AgentFS stores connector-rendered entities as Markdown plus YAML
//! frontmatter. This module parses only the connector-neutral structure:
//! frontmatter envelope, AgentFS identity metadata, stub marker detection, and
...
pub struct Frontmatter {
    pub afs: Option<AfsMetadata>,
    pub title: Option<String>,
    pub properties: FrontmatterProperties,
}

#[derive(Clone, Debug, Default, PartialEq, Eq)]
pub struct AfsMetadata {
...
pub fn parse_directive_line(line: &str, line_number: usize) -> Option<Directive> {
    ...
    if !trimmed.starts_with("::afs") {
        return None;
    }

    let malformed = !trimmed.starts_with("::afs{") || !trimmed.ends_with('}');
    let attributes = if malformed {
        BTreeMap::new()
    } else {
        parse_directive_attributes(&trimmed["::afs{".len()..trimmed.len() - 1])
    };
...
impl RawFrontmatter {
    fn into_frontmatter(self) -> Frontmatter {
        Frontmatter {
            afs: self.afs.map(RawAfsMetadata::into_metadata),
            title: self.title,
...
impl RawAfsMetadata {
    fn into_metadata(self) -> AfsMetadata {
        AfsMetadata {
