...
use afsd::hydration::{HydrationPriority, HydrationQueue, hydration_priority};
...
fn failed_drain_requeues_the_failed_request() {
    ...
    let error = queue
        .drain_ready_with(|_| Err(AfsError::InvalidState("hydrate failed".to_string())))
        .expect_err("drain failure");

    assert_eq!(error, AfsError::InvalidState("hydrate failed".to_string()));
    assert_eq!(queue.len(), 1);
    assert_eq!(
