...
//! exposes AFS as a CLI-shaped tool so sandboxed agents can fall back to MCP
...
Prefer the native `afs` CLI when it is available in your shell. Use the MCP
`afs` tool only when the local agent sandbox cannot execute the host CLI.

Call the MCP tool with arguments exactly as you would pass them after `afs`.
Examples:

- {"argv":["--help"]}
- {"argv":["status","/Users/me/Library/CloudStorage/AFS/notion","--json"]}
- {"argv":["diff","/Users/me/Library/CloudStorage/AFS/notion/Plan/page.md","--json"]}
- {"argv":["pull","/Users/me/Library/CloudStorage/AFS/notion/Plan/page.md","--json"]}
- {"argv":["push","/Users/me/Library/CloudStorage/AFS/notion/Plan/page.md","--yes","--json"]}

Regular online-only files hydrate automatically when opened. Use `afs pull`
...
pub fn discover_afs_binary() -> PathBuf {
    if let Ok(value) = env::var("AFS_MCP_AFS_BIN")
        && !value.trim().is_empty()
    {
    ...
    {
        let sibling = parent.join(binary_name("afs"));
        if sibling.is_file() {
            return sibling;
        }
    }
...
fn initialize_result() -> Value {
    json!({
        ...
        "instructions": "Prefer the native afs CLI when available. If the sandbox cannot execute afs, call the MCP afs tool with argv exactly as CLI arguments after `afs`."
    })
}
        ...
        "tools": [
            {
                "name": "afs",
                "title": "AFS CLI",
                "description": "Run an AFS CLI command on the host. Prefer direct shell usage of `afs` when available; use this MCP fallback when the agent sandbox cannot access the host CLI.",
                "inputSchema": {
                    "type": "object",
                    "properties": {
                        "argv": {
                            "type": "array",
                            "items": { "type": "string" },
                            "description": "Arguments to pass after `afs`, for example [\"status\", \"/path\", \"--json\"]."
                        },
                        "cwd": {
                            "type": "string",
...
fn tools_call_result(params: Option<Value>, config: &McpServerConfig) -> Result<Value, String> {
    ...
    let args: AfsToolArguments = serde_json::from_value(params.arguments)
        .map_err(|error| format!("invalid afs arguments: {error}"))?;
    let execution = execute_afs_tool(args, config)?;
        ...
        "resources": [
            {
                "uri": "afs://help",
                "name": "AFS CLI help",
                "description": "Top-level `afs --help` output.",
                "mimeType": "text/plain"
            },
            {
                "uri": "afs://agent-guide",
                "name": "AFS agent guide",
                "description": "Short instructions for using AFS from sandboxed agents.",
                "mimeType": "text/plain"
            },
            {
                "uri": "afs://mounts",
                "name": "AFS mount info",
                "description": "`afs info --json` output for the current host.",
...
fn resources_read_result(params: Option<Value>, config: &McpServerConfig) -> Result<Value, String> {
    ...
    let (mime_type, text) = match params.uri.as_str() {
        ...
        "afs://agent-guide" => ("text/plain", AGENT_GUIDE.to_string()),
        "afs://mounts" => {
...
fn run_cli(
    ...
    let mut command = Command::new(&config.afs_bin);
    command
        .args(argv)
        .stdin(Stdio::null())
        .stdout(Stdio::piped())
    ...
    let mut child = command
        .spawn()
        .map_err(|error| format!("could not start `{}`: {error}", config.afs_bin.display()))?;

    let started = Instant::now();
    loop {
        if child
            .try_wait()
            .map_err(|error| format!("could not wait for afs: {error}"))?
            .is_some()
        {
            let output = child
                .wait_with_output()
                .map_err(|error| format!("could not read afs output: {error}"))?;
        ...
        if started.elapsed() >= timeout {
            ...
            let output = child
                .wait_with_output()
                .map_err(|error| format!("could not read timed-out afs output: {error}"))?;
            return Ok(CliExecution {
                command: command_display(config, argv),
                exit_code: output.status.code(),
                timed_out: true,
...
fn command_display(config: &McpServerConfig, argv: &[String]) -> Vec<String> {
    std::iter::once(config.afs_bin.display().to_string())
...
mod tests {
    ...
    fn http_post_mcp_returns_initialize_response() {
        ...
        assert!(response.contains("\"serverInfo\":{\"name\":\"afs\""));
    }
    ...
    #[test]
    fn discover_afs_prefers_env_override() {
        let previous = env::var_os("AFS_MCP_AFS_BIN");
        ...
        unsafe {
            env::set_var("AFS_MCP_AFS_BIN", "/tmp/custom-afs");
        }

        assert_eq!(
            discover_afs_binary(),
            std::path::Path::new("/tmp/custom-afs")
        );

        // SAFETY: Restores the environment to its previous value for other tests.
        unsafe {
            if let Some(previous) = previous {
                env::set_var("AFS_MCP_AFS_BIN", previous);
            } else {
                env::remove_var("AFS_MCP_AFS_BIN");
            }
        }
    }
}
