...
Download Locality from the latest finalized GitHub Release. For OS-specific asset names, see [Install Locality](/install).

macOS uses the DMG release asset:

```text
Locality_Mac_v<version>.dmg
```

Linux is supported on Ubuntu 24.04+, Debian 13+, Fedora 40+, openSUSE
Tumbleweed, and Linux Mint 22+. Use the Debian, RPM, or AppImage release asset:

```text
Locality_Linux_v<version>.deb
Locality_Linux_v<version>.rpm
Locality_Linux_v<version>.AppImage
```

DEB and RPM installs enroll the signed Locality repository, so normal
`apt upgrade`, `dnf upgrade`/`yum update`, or `zypper update` runs install later
...
```bash
loc status "$HOME/Library/CloudStorage/Locality/notion-main/Engineering Wiki/Launch Plan/page.md"
loc diff "$HOME/Library/CloudStorage/Locality/notion-main/Engineering Wiki/Launch Plan/page.md"
```

Push a safe reviewed plan:

```bash
loc push "$HOME/Library/CloudStorage/Locality/notion-main/Engineering Wiki/Launch Plan/page.md" -y
```

If the remote changed first, pull before pushing:

```bash
loc pull "$HOME/Library/CloudStorage/Locality/notion-main/Engineering Wiki/Launch Plan/page.md"
loc diff "$HOME/Library/CloudStorage/Locality/notion-main/Engineering Wiki/Launch Plan/page.md"
loc push "$HOME/Library/CloudStorage/Locality/notion-main/Engineering Wiki/Launch Plan/page.md" -y
```
