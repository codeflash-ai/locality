...
When Live Mode is enabled, agents should not run routine `loc pull` or `loc push` after every edit. They should edit the file and stop unless the user explicitly asks them to sync the change back to the source app.
