---
title: "Locality Cloud guide"
description: "Set up a Locality organization, control source access, create a sandbox key, and understand when to use Locality Cloud or Desktop."
---
...
<Frame caption="New teams can create an organization from the Locality Cloud sign-in page. Invited users create an account from the same page.">
  <img src="/images/admin-sign-in.png" alt="Locality Cloud sign-in page with sign in, invited account, and create organization options" />
</Frame>
...
- For **Notion**, select the shared roots offered by your organization. The connector also supports an all-shared scope, which Locality Cloud is rolling out separately; until enabled, explicit root identifiers and the 16-root limit still apply.
- For **Linear**, paste a personal API key created at [Linear API settings](https://linear.app/settings/api). The key is validated server-side and is never stored in browser storage or returned by the API.
...
Organizations with durable hydration enabled see the detailed discovery,
hydration, publishing, and retry phases below. Other organizations see the
existing **Ingesting** state while the same staged rollout is pending.
...
| **Ingesting** | Locality is importing the selected source | Return later and refresh the source status |
| **Discovering content** | Locality is finding provider objects in the selected scope | Return later; discovery continues without the page open |
| **Hydrating X / Y** | Locality is durably importing bounded pages | Progress survives worker restarts and lease changes |
| **Publishing** | The complete durable scope is becoming the ready snapshot | Wait for **Ready** before creating dependent access |
| **Retrying at _time_** | A transient provider or worker failure will retry automatically | No action is required unless retries are exhausted |
| **Ready** | A usable snapshot is available | Continue to **Access** |
| **Needs attention** | Automatic retries are exhausted or setup/authorization needs review | Read the safe reason, copy the diagnostic ID for support, then select **Retry** after fixing the cause |
...
### What is a diagnostic ID?

It identifies one source operation without exposing provider content or credentials. Owners and Admins can copy it from **Operations** or a source card when asking support for help. Include the ID, source name, and approximate time; never include a provider API key.
