...
Create a personal API key at [Linear API settings](https://linear.app/settings/api).
Use a personal API key with access to the teams you want Locality to ingest;
do not paste an OAuth client secret. Locality never stores the plaintext key in
browser storage or writes it to logs.
...
Connection failures are intentionally distinct: an invalid key asks you to
replace it, insufficient permissions asks you to change the key's access,
rate limiting reports when retry is safe, and temporary provider failures are
