...
Register all Google Docs accessible to the connected account.

```bash
loc mount google-docs <path> [--connection <id>] [--mount-id <id>] [--projection <mode>] [--read-only]
```

Docs are projected flat at the mount root, including accessible Shared Drive and shared Docs.
...
loc mount google-docs ~/Locality/google-docs-main --connection google-docs-default
