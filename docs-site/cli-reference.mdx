...
Use this before mounting Google Docs selected through Desktop Google Picker or
explicit `loc mount google-docs --document` values. Locality requests Docs and
`drive.file` access only; it does not enumerate Google Drive or read Drive metadata.
...
Register explicitly selected Google Docs content.

```bash
loc mount google-docs <path> --document <id-or-url> [--document <id-or-url> ...] [--connection <id>] [--mount-id <id>] [--projection <mode>] [--read-only]
```

Use this to mount exactly the Google Docs whose IDs or URLs you provide. In
Desktop, Google Picker provides the equivalent multi-select flow and can
reconfigure an existing mount.

Before starting Desktop, configure Picker with
`LOCALITY_GOOGLE_PICKER_DEVELOPER_KEY` and
`LOCALITY_GOOGLE_PICKER_PROJECT_NUMBER`; the latter is the numeric Google Cloud
project number for the Picker API key and OAuth client.
...
| --- | --- |
| `<path>` | Local directory to register as the mount. |
| `--document <id-or-url>` | Google Docs document ID or URL. Repeat for each selected document. |
...
```bash
loc mount google-docs ~/Locality/google-docs-main --document <document-id> --projection plain-files
loc mount google-docs ~/Locality/google-docs-main --document <document-url> --document <second-document-id> --connection google-docs-default
