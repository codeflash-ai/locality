...
private enum Command {
  ...
  static func parse(_ arguments: [String]) throws -> Self {
    ...
    guard let action = args.first else {
      throw UsageError(
        "usage: agentfs-file-providerctl register|open|unregister|list|reset [options]")
    }
  ...
  func run() throws -> FileProviderCtlReport {
    ...
    case .open(let mountId):
      ...
      guard FileManager.default.fileExists(atPath: url.path) else {
        throw UsageError(
          "File Provider domain \(mountId) exists but macOS has not created \(url.path). Enable the AgentFS File Provider extension in System Settings, then try again."
        )
      }
      return FileProviderCtlReport(
        ok: true,
