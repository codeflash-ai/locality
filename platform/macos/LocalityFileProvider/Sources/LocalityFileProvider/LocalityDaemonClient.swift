...
enum LocalityDaemonClientError: Error, LocalizedError {
  ...
  var errorDescription: String? {
    ...
    case .connectFailed(let message):
      return "failed to connect to localityd: \(message)"
    case .writeFailed:
...
struct LocalityDaemonError: Decodable {
  let code: String
  let message: String
}

struct LocalityDaemonResponse<Payload: Decodable>: Decodable {
  let ok: Bool
  let payload: Payload?
  let error: LocalityDaemonError?
}

struct LocalityItemPayload: Decodable {
  let mountId: String
  let item: LocalityItemMetadata
...
struct LocalityChildrenPayload: Decodable {
  let mountId: String
  let containerIdentifier: String
  let children: [LocalityItemMetadata]
...
struct LocalityDomainChildrenPayload: Decodable {
  let domainId: String
  let children: [LocalityDomainChild]
...
struct LocalityDomainChild: Decodable {
  let mountId: String
  let item: LocalityItemMetadata
...
struct LocalityMaterializePayload: Decodable {
  let mountId: String
...
struct LocalityReadPayload: Decodable {
  let mountId: String
  let identifier: String
  let remoteId: String
  let path: String
...
struct LocalityWritePayload: Decodable {
  let mountId: String
  let identifier: String
  let remoteId: String
  let path: String
...
struct LocalityMutationPayload: Decodable {
  let mountId: String
  let identifier: String
  let path: String
  let item: LocalityItemMetadata
...
struct LocalityItemMetadata: Decodable {
  let identifier: String
...
final class LocalityDaemonClient: @unchecked Sendable {
  ...
  init(socketPath: String? = nil, tcpAddress: String? = nil) throws {
    ...
    if let socketPath = ProcessInfo.processInfo.environment["LOCALITY_DAEMON_SOCKET"] {
      self.transport = .unixSocket(socketPath)
      return
    }
    let address =
      tcpAddress
      ?? ProcessInfo.processInfo.environment["LOCALITY_DAEMON_TCP_ADDR"]
      ?? "127.0.0.1:38567"
    self.transport = try LocalityDaemonTransport.parseTcpAddress(address)
  }

  static func unixSocketFromHome() throws -> LocalityDaemonClient {
    guard let home = ProcessInfo.processInfo.environment["HOME"] else {
      throw LocalityDaemonClientError.homeDirectoryUnavailable
    }
    return try LocalityDaemonClient(socketPath: "\(home)/.loc/localityd.sock")
  }

  func item(mountId: String, identifier: String) throws -> LocalityItemPayload {
    try request([
  ...
  func read(mountId: String, identifier: String) throws -> LocalityReadPayload {
    try request([
  ...
  func createFile(
    ...
    filename: String
  ) throws -> LocalityMutationPayload {
    try request([
  ...
  private func request<Payload: Decodable>(_ object: [String: String]) throws -> Payload {
    ...
    try payload.withUnsafeBytes { rawBuffer in
      guard let baseAddress = rawBuffer.baseAddress else {
        throw LocalityDaemonClientError.writeFailed
      }
      var written = 0
    ...
    let response = try JSONDecoder().decode(LocalityDaemonResponse<Payload>.self, from: responseData)
  ...
  private func connectUnixSocket(_ socketPath: String) throws -> Int32 {
    ...
    if fd < 0 {
      throw LocalityDaemonClientError.connectFailed(String(cString: strerror(errno)))
    }
    ...
    guard socketPath.utf8.count < maxPathLength else {
      close(fd)
      throw LocalityDaemonClientError.socketPathTooLong(socketPath)
    }
    ...
    if result != 0 {
      ...
      close(fd)
      throw LocalityDaemonClientError.connectFailed(message)
    }
  ...
  private func connectTcp(host: String, port: UInt16) throws -> Int32 {
    ...
    guard inet_pton(AF_INET, host, &address.sin_addr) == 1 else {
      close(fd)
      throw LocalityDaemonClientError.invalidDaemonAddress("\(host):\(port)")
    }
  ...
  private func readLine(fd: Int32) throws -> Data {
    ...
    while true {
      ...
      if count < 0 {
        throw LocalityDaemonClientError.readFailed
      }
      if count == 0 {
        break
      }
...
private enum LocalityDaemonTransport {
  ...
  static func parseTcpAddress(_ address: String) throws -> Self {
    ...
    guard parts.count == 2, let port = UInt16(parts[1]) else {
      throw LocalityDaemonClientError.invalidDaemonAddress(address)
    }
