...
async fn activate_macos_file_provider_for_user_action(
    ...
    #[cfg(target_os = "macos")]
    {
        let options = macos_file_provider_activation_options(action);
        if !options.register_domain {
            return Ok(MacosFileProviderActivation::NotRequested);
        }
        debug_assert!(!options.reveal_cloudstorage_root);
        debug_assert!(options.warm_extension_after_registration);

        if options.prepare_disabled_domain_retry {
...
#[cfg(target_os = "macos")]
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
struct MacosFileProviderActivationOptions {
    register_domain: bool,
    prepare_disabled_domain_retry: bool,
    warm_extension_after_registration: bool,
    reveal_cloudstorage_root: bool,
}

#[cfg(target_os = "macos")]
fn macos_file_provider_activation_options(
    action: WorkspaceMountOnboardingAction,
) -> MacosFileProviderActivationOptions {
    match action {
        WorkspaceMountOnboardingAction::Start => MacosFileProviderActivationOptions {
            register_domain: true,
            prepare_disabled_domain_retry: false,
            warm_extension_after_registration: true,
            reveal_cloudstorage_root: false,
        },
        WorkspaceMountOnboardingAction::AllowInMacos => MacosFileProviderActivationOptions {
            register_domain: true,
            prepare_disabled_domain_retry: true,
            warm_extension_after_registration: true,
            reveal_cloudstorage_root: false,
        },
        WorkspaceMountOnboardingAction::Restore | WorkspaceMountOnboardingAction::CheckAgain => {
            MacosFileProviderActivationOptions {
                ...
                prepare_disabled_domain_retry: false,
                warm_extension_after_registration: false,
                reveal_cloudstorage_root: false,
            }
        }
    }
...
mod tests {
    ...
    #[cfg(target_os = "macos")]
    #[test]
    fn allow_in_macos_retries_registration_without_finder_reveal() {
        let options = super::macos_file_provider_activation_options(
            super::WorkspaceMountOnboardingAction::AllowInMacos,
        );

        assert!(options.prepare_disabled_domain_retry);
        assert!(options.warm_extension_after_registration);
        assert!(!options.reveal_cloudstorage_root);
    }

    #[cfg(target_os = "macos")]
    #[test]
    fn restore_and_check_again_skip_native_registration() {
        assert!(
            !super::macos_file_provider_activation_options(
                super::WorkspaceMountOnboardingAction::Restore,
            )
            .register_domain
        );
